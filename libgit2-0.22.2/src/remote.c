...
static int add_refspec_to(git_vector *vector, const char *string, bool is_fetch)
{
	...
	spec = (git_refspec *) git__calloc(1, sizeof(git_refspec));
...
static int create_internal(git_remote **out, git_repository *repo, const char *name, const char *url, const char *fetch)
{
	...
	remote = (git_remote *) git__calloc(1, sizeof(git_remote));
...
int git_remote_dup(git_remote **dest, git_remote *source)
{
	...
	git_remote *remote = (git_remote *) git__calloc(1, sizeof(git_remote));
	GITERR_CHECK_ALLOC(remote);
	...
	git_config_foreach_cb cb, void *payload)
{
	...
	if (cb != NULL)
		...
		error = git_config_get_string((const char**)payload, config, key);
...
int git_remote_lookup(git_remote **out, git_repository *repo, const char *name)
{
	...
	remote = (git_remote*) git__malloc(sizeof(git_remote));
	GITERR_CHECK_ALLOC(remote);
...
static int update_config_refspec(const git_remote *remote, git_config *config, int direction)
{
	...
	for (i = 0; i < remote->refspecs.length; i++) {
		git_refspec *spec = (git_refspec*) git_vector_get(&remote->refspecs, i);
...
static int dwim_refspecs(git_vector *out, git_vector *refspecs, git_vector *refs)
{
	...
	git_vector_foreach(refspecs, i, spec, git_refspec*) {
...
static void free_refspecs(git_vector *vec)
{
	...
	git_vector_foreach(vec, i, spec, git_refspec *) {
		git_refspec__free(spec);
		git__free(spec);
	}
...
static int remote_head_for_fetchspec_src(git_remote_head **out, git_vector *update_heads, const char *fetchspec_src)
{
	...
	*out = NULL;
	...
	git_vector_foreach(update_heads, i, remote_ref, git_remote_head *) {
...
cleanup:
	for (i = 0; i < fetchhead_refs.length; ++i)
		git_fetchhead_ref_free((git_fetchhead_ref*)fetchhead_refs.contents[i]);
...
int git_remote_prune(git_remote *remote)
{
	 ...
	 */
	git_vector_foreach(&candidates, i, refname, char*) {
		git_vector_foreach(&remote->active_refspecs, j, spec, git_refspec*) {
			git_buf buf = GIT_BUF_INIT;
			size_t pos;
			char *src_name;
		...
		const char *log_message)
{
	...
	for (; i < refs->length; ++i) {
		head = (git_remote_head*) git_vector_get(refs, i);
		autotag = 0;
		     ...
		     size_t *out_i, size_t *out_j, size_t *out_k)
{
	...
	for (; i < refs->length; i++) {
		...
		for (; j < active->length; j++) {
			spec = (git_refspec*) git_vector_get(active, j);

			if (!git_refspec_src_matches(spec, head->name))
				continue;

			for (; k < passive->length; k++) {
				passive_spec = (git_refspec*) git_vector_get(passive, k);
		...
		const char *reflog_message)
{
	...
	git_vector_foreach(&remote->active_refspecs, i, spec, git_refspec*) {
		if (spec->push)
			continue;
...
{
	git_vector *list = (git_vector *) payload;
	...
	const char *new_remote_name)
{
	...
	git_buf nnamespace = GIT_BUF_INIT, old_namespace = GIT_BUF_INIT;
	git_buf new_name = GIT_BUF_INIT;
	git_buf log_message = GIT_BUF_INIT;
	size_t pfx_len;
	const char *target;

	if ((error = git_buf_printf(&nnamespace, GIT_REFS_REMOTES_DIR "%s/", new_remote_name)) < 0)
		return error;

	pfx_len = strlen(GIT_REFS_REMOTES_DIR) + strlen(old_remote_name) + 1;
	git_buf_puts(&new_name, nnamespace.ptr);
...
static int rename_fetch_refspecs(git_vector *problems, git_remote *remote, const char *new_name)
{
	...
	git_vector_foreach(&remote->refspecs, i, spec, git_refspec *) {
		if (spec->push)
			continue;
	...
	if (error < 0) {
		char *str;
		git_vector_foreach(problems, i, str, char*)
			git__free(str);
...
static int set_refspecs(git_remote *remote, git_strarray *array, int push)
{
	...
	for (i = 0; i < vec->length; i++) {
		spec = (git_refspec *) git_vector_get(vec, i);
		if (spec->push != push)
			continue;
...
{
	return (git_refspec *)git_vector_get(&remote->refspecs, n);
}
...
static int remove_refs(git_repository *repo, const git_refspec *spec)
{
	...
	git_vector_foreach(&refs, i, name, char*) {
...
cleanup:
	git_reference_iterator_free(iter);
	git_vector_foreach(&refs, i, dup, char*) {
		git__free(dup);
	}
	git_vector_free(&refs);
	return error;
