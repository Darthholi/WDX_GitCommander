...
{
	git_attr_file *attrs = (git_attr_file *) git__calloc(1, sizeof(git_attr_file));
...
int git_attr_file__clear_rules(git_attr_file *file, bool need_lock)
{
	...
	git_vector_foreach(&file->rules, i, rule, git_attr_rule*)
	...
	git_attr_file_parser parser)
{
	...
	*out = NULL;
	...
	case GIT_ATTR_FILE__FROM_INDEX: {
		...
		git_buf_put(&content,(char*) git_blob_rawcontent(blob), git_blob_rawsize(blob));
		break;
	}
	...
	git_repository *repo, git_attr_file *attrs, const char *data)
{
	...
	while (!error && *scan) {
		...
		if (!rule && !(rule = (git_attr_rule*) git__calloc(1, sizeof(*rule)))) {
			error = -1;
			break;
		}
	...
	const char **value)
{
	...
	*value = NULL;
	...
	git_attr_file__foreach_matching_rule(file, path, i, rule, git_attr_rule* ) {
  //for ((i) = (&file->rules)->length - 1; (i) < SIZE_MAX && ((rule) = (git_attr_rule*) (&file->rules)->contents[i], 1); i-- )
	...
	git_attr_rule *rule, const char *name)
{
	...
	return (git_attr_assignment *)git_vector_get(&rule->assigns, pos);
}
	...
	const char **base)
{
	...
	*base = scan;
	...
	if (context) {
		char *slash = (char*)strchr(context, '/');
		size_t len;
		if (slash) {
...
static int sort_by_hash_and_name(const void *a_raw, const void *b_raw)
{
	const git_attr_name *a = (git_attr_name *)a_raw;
	const git_attr_name *b = (git_attr_name *)b_raw;
...
static int merge_assignments(void **old_raw, void *new_raw)
{
	...
	git_attr_assignment *pnew = (git_attr_assignment *)new_raw;
	...
	const char **base)
{
	...
	while (*scan && *scan != '\n') {
		...
		if (!assign) {
			assign = (git_attr_assignment *) git__calloc(1, sizeof(git_attr_assignment));
		...
		if (repo != NULL && assign->value == git_attr__true) {
			...
			if (macro != NULL) {
				...
				git_vector_foreach(&macro->assigns, i, massign, git_attr_assignment *) {
...
static void git_attr_rule__clear(git_attr_rule *rule)
{
	...
	if (!(rule->match.flags & GIT_ATTR_FNMATCH_IGNORE)) {
		git_vector_foreach(&rule->assigns, i, assign,git_attr_assignment *)
