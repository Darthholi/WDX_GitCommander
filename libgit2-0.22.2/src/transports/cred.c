	...
	const char *passphrase)
{
	...
	c = (git_cred_ssh_key *) git__calloc(1, sizeof(git_cred_ssh_key));
	GITERR_CHECK_ALLOC(c);
	...
	void *payload)
{
	...
	c = (git_cred_ssh_interactive *) git__calloc(1, sizeof(git_cred_ssh_interactive));
	GITERR_CHECK_ALLOC(c);
	...
	void *payload)
{
	...
	c = (git_cred_ssh_custom *) git__calloc(1, sizeof(git_cred_ssh_custom));
	GITERR_CHECK_ALLOC(c);
	...
	if (publickey_len > 0) {
		c->publickey = (char*) git__malloc(publickey_len);
		GITERR_CHECK_ALLOC(c->publickey);
...
int git_cred_default_new(git_cred **cred)
{
	...
	c = (git_cred*) git__calloc(1, sizeof(git_cred_default));
	GITERR_CHECK_ALLOC(c);
...
int git_cred_username_new(git_cred **cred, const char *username)
{
	...
	len = strlen(username);
	c = (git_cred_username *) git__malloc(sizeof(git_cred_username) + len + 1);
	GITERR_CHECK_ALLOC(c);
