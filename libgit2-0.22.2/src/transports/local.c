...
static void free_heads(git_vector *heads)
{
	...
	git_vector_foreach(heads, i, head, git_remote_head *)
		free_head(head);
...
static int store_refs(transport_local *t)
{
	...
	git_vector_foreach(&t->refs, i, head, git_remote_head *) {
		git__free(head->name);
		git__free(head);
	}
	...
	size_t count)
{
	...
	/* Fill in the loids */
	git_vector_foreach(&t->refs, i, rhead, git_remote_head *) {
		git_object *obj;
	...
	git_push *push)
{
	...
	git_vector_foreach(&push->specs, j, spec, push_spec*) {
		push_status *status;
...
int git_transport_local(git_transport **out, git_remote *owner, void *param)
{
	...
	t = (transport_local*) git__calloc(1, sizeof(transport_local));
