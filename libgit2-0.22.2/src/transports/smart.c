...
int git_smart__update_heads(transport_smart *t, git_vector *symrefs)
{
	...
	git_vector_foreach(&t->refs, i, pkt, git_pkt *) {
		...
		if (symrefs) {
			...
			git_vector_foreach(symrefs, j, spec, git_refspec*) {
				git_buf_clear(&buf);
...
static void free_symrefs(git_vector *symrefs)
{
	...
	git_vector_foreach(symrefs, i, spec, git_refspec*) {
...
static int git_smart__close(git_transport *transport)
{
	...
	git_vector_foreach(common, i, p, git_pkt*)
		git_pkt_free(p);
...
static void git_smart__free(git_transport *transport)
{
	...
	git_vector_foreach(refs, i, p, git_pkt*)
		git_pkt_free(p);
...
{
	const git_pkt_ref *ref_a = (git_pkt_ref *) a, *ref_b = (git_pkt_ref *) b;
