...
int git_smart__store_refs(transport_smart *t, int flushes)
{
	 ...
	 */
	git_vector_foreach(refs, i, pkt,git_pkt *) {
		git_pkt_free(pkt);
	}
...
static int append_symref(const char **out, git_vector *symrefs, const char *ptr)
{
	...
	mapping = (git_refspec*) git__calloc(1, sizeof(git_refspec));
...
int git_smart__negotiate_fetch(git_transport *transport, git_repository *repo, const git_remote_head * const *wants, size_t count)
{
	...
	while (i < 256) {
		...
		if (i % 20 == 0 && t->rpc) {
			...
			git_vector_foreach(&t->common, i, pkt,git_pkt_ack *) {
...
static int gen_pktline(git_buf *buf, git_push *push)
{
	...
	git_vector_foreach(&push->specs, i, spec,push_spec *) {
...
static int add_push_report_pkt(git_push *push, git_pkt *pkt)
{
	...
	switch (pkt->type) {
		case GIT_PKT_OK:
			status = (push_status *) git__calloc(sizeof(push_status), 1);
...
{
	git_pkt_ref *added = (git_pkt_ref*) git__calloc(1, sizeof(git_pkt_ref));
...
{
	git_pkt_ref *ref;
	push_spec *xpush_spec;
	push_status *xpush_status;
	size_t i, j, refs_len;
	int cmp;
	...
	git_vector_foreach(push_specs, i, xpush_spec, push_spec*) {
		xpush_status = (push_status*) git_vector_get(push_report, i);

		/* For each push spec we sent to the server, we should have
		 * gotten back a status packet in the push report which matches */
		if (strcmp(xpush_spec->refspec.dst, xpush_status->ref)) {
			giterr_set(GITERR_NET, "report-status: protocol error");
			return -1;
		}
	}
	...
	while (i < push_specs->length && j < refs_len) {
		xpush_spec = (push_spec*) git_vector_get(push_specs, i);
		xpush_status = (push_status*) git_vector_get(push_report, i);
		ref = (git_pkt_ref*) git_vector_get(refs, j);

		cmp = strcmp(xpush_spec->refspec.dst, ref->head.name);
		...
		if (cmp < 0 &&
			!xpush_status->msg &&
			add_ref_from_push_spec(refs, xpush_spec) < 0)
			return -1;

		/* Update case, delete case */
		if (cmp == 0 &&
			!xpush_status->msg)
			git_oid_cpy(&ref->head.oid, &xpush_spec->loid);
	}
	...
	git_vector_rforeach(refs, i, ref, git_pkt_ref* ) {
...
{
	int error = 0;
	struct push_packbuilder_payload *payload = (push_packbuilder_payload *) data;
