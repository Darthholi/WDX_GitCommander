...
int git_buf_text_crlf_to_lf(git_buf *tgt, const git_buf *src)
{
	...
	const char *next = (char*) memchr(scan, '\r', src->size);
	char *out;
	...
	for (; next; scan = next + 1, next = (char*) memchr(scan, '\r', scan_end - scan)) {
		if (next > scan) {
...
int git_buf_text_lf_to_crlf(git_buf *tgt, const git_buf *src)
{
	...
	const char *next = (char*) memchr(scan, '\n', src->size);
	...
	for (; next; scan = next + 1, next = (char*) memchr(scan, '\n', end - scan)) {
