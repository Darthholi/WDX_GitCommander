	...
	git_revwalk *walk, const git_oid *oid)
{
	...
	if (pos != kh_end(walk->commits))
		return (git_commit_list_node *)kh_value(walk->commits, pos);
	...
	do {
		...
		for (i = 0; i < commit->out_degree; ++i)
			if (!commit->parents[i]->uninteresting) {
				git_commit_list_node **node = (git_commit_list_node **) git_array_alloc(pending);
...
static int revwalk_next_timesort(git_commit_list_node **object_out, git_revwalk *walk)
{
	...
	while ((next = (git_commit_list_node *) git_pqueue_pop(&walk->iterator_time)) != NULL)
...
static int premark_uninteresting(git_revwalk *walk)
{
	...
	while (interesting(&q)) {
		commit = (git_commit_list_node *) git_pqueue_pop(&q);
...
int git_revwalk_new(git_revwalk **revwalk_out, git_repository *repo)
{
	...
	walk = (git_revwalk*) git__malloc(sizeof(git_revwalk));
...
void git_revwalk_reset(git_revwalk *walk)
{
	...
	kh_foreach_value(walk->commits, commit, git_commit_list_node *, {
		commit->seen = 0;
		commit->in_degree = 0;
		commit->topo_delay = 0;
