...
int git_mwindow_get_pack(struct git_pack_file **out, const char *path)
{
	...
	if (git_strmap_valid_index(git__pack_cache, pos)) {
		pack = (git_pack_file*) git_strmap_value_at(git__pack_cache, pos);
...
static int git_mwindow_close_lru(git_mwindow_file *mwf)
{
	...
	for (i = 0; i < ctl->windowfiles.length; ++i) {
		...
		git_mwindow_file *cur = (git_mwindow_file *) git_vector_get(&ctl->windowfiles, i);
	...
	git_off_t offset)
{
	...
	w = (git_mwindow*) git__malloc(sizeof(*w));
...
void git_mwindow_file_deregister(git_mwindow_file *mwf)
{
	...
	git_vector_foreach(&ctl->windowfiles, i, cur, git_mwindow_file *) {
		if (cur == mwf) {
			git_vector_remove(&ctl->windowfiles, i);
			git_mutex_unlock(&git__mwindow_mutex);
			return;
