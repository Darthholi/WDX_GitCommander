	...
	git_attr_cache *cache, const char *path)
{
	...
	if (git_strmap_valid_index(cache->files, pos))
		return (git_attr_file_entry *) git_strmap_value_at(cache->files, pos);
	else
		return NULL;
}
	...
	git_pool *pool)
{
	...
	ce = (git_attr_file_entry *) git_pool_mallocz(pool, (uint32_t)cachesize);
...
static int attr_cache_upsert(git_attr_cache *cache, git_attr_file *file)
{
	...
	old = (git_attr_file*) git__compare_and_swap(
...
static int attr_cache_remove(git_attr_cache *cache, git_attr_file *file)
{
	...
	if ((entry = attr_cache_lookup_entry(cache, file->entry->path)) != NULL)
		file = (git_attr_file*) git__compare_and_swap(&entry->file[file->source], file, NULL);
	...
	const char *filename)
{
	...
	entry = (git_attr_file_entry *) git_strmap_value_at(files, pos);
...
static void attr_cache__free(git_attr_cache *cache)
{
	...
	if (cache->files != NULL) {
		...
		git_strmap_foreach_value(cache->files, entry, git_attr_file_entry *, {
			for (i = 0; i < GIT_ATTR_FILE_NUM_SOURCES; ++i) {
				if ((file = (git_attr_file*) git__swap(entry->file[i], NULL)) != NULL) {
					GIT_REFCOUNT_OWN(file, NULL);
					git_attr_file__free(file);
				}
			}
	...
	if (cache->macros != NULL) {
		...
		git_strmap_foreach_value(cache->macros, rule, git_attr_rule *, {
...
int git_attr_cache__do_init(git_repository *repo)
{
	...
	cache = (git_attr_cache *) git__calloc(1, sizeof(git_attr_cache));
	...
	cache = (git_attr_cache *) git__compare_and_swap(&repo->attrcache, NULL, cache);
	if (cache)
...
void git_attr_cache_flush(git_repository *repo)
{
	 ...
	 */
	if (repo && (cache = (git_attr_cache *) git__swap(repo->attrcache, NULL)) != NULL)
		attr_cache__free(cache);
}
