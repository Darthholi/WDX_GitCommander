...
int git_packbuilder_new(git_packbuilder **out, git_repository *repo)
{
	...
	*out = NULL;
	...
	pb = (git_packbuilder*) git__calloc(1, sizeof(*pb));
			   ...
			   const char *name)
{
	...
	if (pb->nr_objects >= pb->nr_alloc) {
		...
		pb->object_list = (git_pobject*) git__realloc(pb->object_list,
	...
	void *cb_data)
{
	...
	if (po->z_delta_size) {
		...
		zbuf = (unsigned char*) git__malloc(zbuf_len);
...
{
	git_packbuilder *pb = (git_packbuilder *) data;
	git_pobject *po;
	khiter_t pos;
	...
	po = (git_pobject*) kh_value(pb->object_ix, pos);
	po->tagged = 1;
...
static git_pobject **compute_write_order(git_packbuilder *pb)
{
	...
	git_pobject **wo = (git_pobject**) git__malloc(sizeof(*wo) * pb->nr_objects);
...
static int write_pack_buf(void *buf, size_t size, void *data)
{
	...
	return git_buf_put(b, (char*) buf, size);
}
		       ...
		       int depth)
{
	...
	array = (unpacked*) git__calloc(window, sizeof(struct unpacked));
...
static int prepare_pack(git_packbuilder *pb)
{
	...
	delta_list = (git_pobject**) git__malloc(pb->nr_objects * sizeof(*delta_list));
...
{
	struct pack_write_context *ctx = (pack_write_context *) payload;
...
{
	int error;
	struct tree_walk_context *ctx = (tree_walk_context *) payload;
