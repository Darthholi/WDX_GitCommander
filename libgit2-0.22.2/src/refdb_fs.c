...
typedef struct refdb_fs_backend {
 ...
 int packref_cmp(const void *a_, const void *b_)
{
	const struct packref *a = (packref*) a_, *b = (packref*) b_;
	return strcmp(a->name, b->name);
}

 int packed_reload(refdb_fs_backend *backend)
{
	int error;
	...
	if (*scan == '#') {
		 const char *traits_header = "# pack-refs with: ";
...
corrupted:
 ...
 int loose_readbuffer(git_buf *buf, const char *base, const char *path)
{
	int error;
 ...
 int loose_lookup_to_packfile(refdb_fs_backend *backend, const char *name)
{
	int error = 0;
 ...
 int _dirent_loose_load(void *payload, git_buf *full_path)
{
	refdb_fs_backend *backend = (refdb_fs_backend *) payload;
 ...
 * disk.
 */
 int packed_loadloose(refdb_fs_backend *backend)
{
	int error;
	...
	const char *ref_name)
{
 ...
 const char *loose_parse_symbolic(git_buf *file_content)
{
	...
	const char *ref_name)
{
 ...
 int ref_error_notfound(const char *name)
{
	...
	const char *ref_name)
{
	...
	entry = (packref*) git_sortedcache_lookup(backend->refcache, ref_name);
	if (!entry) {
...
void refdb_fs_backend__iterator_free(git_reference_iterator *_iter)
{
 ...
 int iter_load_loose_paths(refdb_fs_backend *backend, refdb_fs_iter *iter)
{
	int error = 0;
	...
	while (!error && !git_iterator_advance(&entry, fsit)) {
		...
		ref = (packref*) git_sortedcache_lookup(backend->refcache, ref_name);
		if (ref)
	...
	git_reference **out, git_reference_iterator *_iter)
{
	...
	while (iter->loose_pos < iter->loose.length) {
		const char *path = (char*) git_vector_get(&iter->loose, iter->loose_pos++);
	...
	while (iter->packed_pos < git_sortedcache_entrycount(iter->cache)) {
		ref = (packref*) git_sortedcache_entry(iter->cache, iter->packed_pos++);
	...
	const char **out, git_reference_iterator *_iter)
{
 ...
 int refdb_fs_backend__iterator(
	git_reference_iterator **out, git_refdb_backend *_backend, const char *glob)
{
	...
	iter = (refdb_fs_iter*) git__calloc(1, sizeof(refdb_fs_iter));
	...
	int force)
{
	...
	for (i = 0; i < git_sortedcache_entrycount(backend->refcache); ++i) {
		struct packref *ref = (packref*) git_sortedcache_entry(backend->refcache, i);
 ...
 int loose_lock(git_filebuf *file, refdb_fs_backend *backend, const char *name)
{
	int error;
	git_buf ref_path = GIT_BUF_INIT;
 ...
 int loose_commit(git_filebuf *file, const git_reference *ref)
{
	assert(file && ref);
 ...
 int refdb_fs_backend__lock(void **out, git_refdb_backend *_backend, const char *refname)
{
	int error;
	git_filebuf *lock;
	refdb_fs_backend *backend = (refdb_fs_backend *) _backend;

	lock = (git_filebuf*) git__calloc(1, sizeof(git_filebuf));
 ...
 int refdb_fs_backend__delete_tail(
	git_refdb_backend *_backend,
	git_filebuf *file,
	const char *ref_name,
	const git_oid *old_id, const char *old_target);

 int refdb_fs_backend__unlock(git_refdb_backend *backend, void *payload, int success, int update_reflog,
 ...
 */
 int packed_find_peel(refdb_fs_backend *backend, struct packref *ref)
{
	git_object *object;
 ...
 * here otherwise.
 */
 int packed_remove_loose(refdb_fs_backend *backend)
{
	size_t i;
 ...
 */
 int packed_write(refdb_fs_backend *backend)
{
	...
	for (i = 0; i < git_sortedcache_entrycount(refcache); ++i) {
		struct packref *ref = (packref*) git_sortedcache_entry(refcache, i);
...
fail:
 ...
 int reflog_append(refdb_fs_backend *backend, const git_reference *ref, const git_oid *old, const git_oid *pnew, const git_signature *author, const char *message);
 int has_reflog(git_repository *repo, const char *name);

/* We only write if it's under heads/, remotes/ or notes/ or if it already has a log */
 int should_write_reflog(int *write, git_repository *repo, const char *name)
{
	int error, logall;
 ...
 int cmp_old_ref(int *cmp, git_refdb_backend *backend, const char *name,
	const git_oid *old_id, const char *old_target)
{
	int error = 0;
 ...
 */
 int maybe_append_head(refdb_fs_backend *backend, const git_reference *ref, const git_signature *who, const char *message)
{
	int error;
	git_oid old_id = {{0}};
...
cleanup:
 ...
 int refdb_reflog_fs__rename(git_refdb_backend *_backend, const char *old_name, const char *new_name);

 int refdb_fs_backend__rename(
	git_reference **out,
	git_refdb_backend *_backend,
	const char *old_name,
	const char *new_name,
	...
	const char *message)
{
	...
	pnew = git_reference__set_name(old, new_name);
	if (!pnew) {
		git_reference_free(old);
		return -1;
	}

	if ((error = loose_lock(&file, backend, pnew->name)) < 0) {
		git_reference_free(pnew);
		return error;
	}
	...
	if (((error == 0) || (error == GIT_ENOTFOUND)) &&
	    ((error = reflog_append(backend, pnew, NULL, NULL, who, message)) < 0)) {
		git_reference_free(pnew);
		git_filebuf_cleanup(&file);
		return error;
	}
	...
	if ((error = loose_commit(&file, pnew)) < 0 || out == NULL) {
		git_reference_free(pnew);
		return error;
	}
 ...
 int refdb_fs_backend__compress(git_refdb_backend *_backend)
{
	refdb_fs_backend *backend = (refdb_fs_backend *)_backend;
 ...
 void refdb_fs_backend__free(git_refdb_backend *_backend)
{
	refdb_fs_backend *backend = (refdb_fs_backend *)_backend;
 ...
 int setup_namespace(git_buf *path, git_repository *repo)
{
	...
	if (repo->nnamespace == NULL)
		return 0;

	parts = end = git__strdup(repo->nnamespace);
	if (parts == NULL)
		return -1;
 ...
 int reflog_alloc(git_reflog **reflog, const char *name)
{
	git_reflog *log;

	*reflog = NULL;

	log = (git_reflog *) git__calloc(1, sizeof(git_reflog));
 ...
 int reflog_parse(git_reflog *log, const char *buf, size_t buf_size)
{
	const char *ptr;
...
#define seek_forward(_increase) do { \
	...
	while (buf_size > GIT_REFLOG_SIZE_MIN) {
		entry = (git_reflog_entry *) git__calloc(1, sizeof(git_reflog_entry));
		GITERR_CHECK_ALLOC(entry);

		entry->committer = (git_signature*) git__malloc(sizeof(git_signature));
...
GIT_INLINE(int) retrieve_reflog_path(git_buf *path, git_repository *repo, const char *name)
{
 ...
 int refdb_reflog_fs__ensure_log(git_refdb_backend *_backend, const char *name)
{
	refdb_fs_backend *backend;
	git_repository *repo;
	git_buf path = GIT_BUF_INIT;
...
cleanup:
 ...
 int refdb_reflog_fs__has_log(git_refdb_backend *_backend, const char *name)
{
	refdb_fs_backend *backend;
 ...
 int refdb_reflog_fs__read(git_reflog **out, git_refdb_backend *_backend, const char *name)
{
	int error = -1;
	...
	const char *msg)
{
 ...
 int lock_reflog(git_filebuf *file, refdb_fs_backend *backend, const char *refname)
{
	git_repository *repo;
	git_buf log_path = GIT_BUF_INIT;
	int error;
...
cleanup:
 ...
 int refdb_reflog_fs__write(git_refdb_backend *_backend, git_reflog *reflog)
{
	int error = -1;
	unsigned int i;
	git_reflog_entry *entry;
	...
	git_vector_foreach(&reflog->entries, i, entry, git_reflog_entry *) {
...
/* Append to the reflog, must be called under reference lock */
 int reflog_append(refdb_fs_backend *backend, const git_reference *ref, const git_oid *old, const git_oid *pnew, const git_signature *who, const char *message)
{
...
cleanup:
 ...
 int refdb_reflog_fs__delete(git_refdb_backend *_backend, const char *name)
{
	int error;
	git_buf path = GIT_BUF_INIT;
	...
	git_repository *repository)
{
	...
	backend = (refdb_fs_backend *) git__calloc(1, sizeof(refdb_fs_backend));
	...
	if (!git_repository__cvar(&t, backend->repo, GIT_CVAR_IGNORECASE) && t) {
		backend->iterator_flags = 	backend->iterator_flags | GIT_ITERATOR_IGNORE_CASE;
		backend->direach_flags  |= GIT_PATH_DIR_IGNORE_CASE;
	}
	if (!git_repository__cvar(&t, backend->repo, GIT_CVAR_PRECOMPOSE) && t) {
		backend->iterator_flags = backend->iterator_flags | GIT_ITERATOR_PRECOMPOSE_UNICODE;
