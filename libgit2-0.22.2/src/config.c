...
static void config_free(git_config *cfg)
{
	...
	for (i = 0; i < cfg->files.length; ++i) {
		internal = (file_internal*) git_vector_get(&cfg->files, i);
...
int git_config_new(git_config **out)
{
	...
	cfg = (git_config*) git__malloc(sizeof(git_config));
...
int git_config_snapshot(git_config **out, git_config *in)
{
	...
	*out = NULL;
	...
	git_vector_foreach(&in->files, i, internal, file_internal*) {
	...
	git_config_level_t level)
{
	...
	if (level == GIT_CONFIG_HIGHEST_LEVEL) {
		...
		git_vector_foreach(&cfg->files, i, internal, file_internal*) {
	...
	*internal_out = (file_internal*) git_vector_get(&cfg->files, pos);
	...
	git_config_level_t level)
{
	...
	internal = (file_internal*) git_vector_get(&cfg->files, pos);
	...
	int force)
{
	...
	internal = (file_internal*) git__malloc(sizeof(file_internal));
...
static int find_next_backend(size_t *out, const git_config *cfg, size_t i)
{
	...
	for (; i > 0; --i) {
		internal = (file_internal*) git_vector_get(&cfg->files, i - 1);
...
static int all_iter_next(git_config_entry **entry, git_config_iterator *_iter)
{
	...
	do {
		...
		internal = (file_internal*) git_vector_get(&iter->cfg->files, i - 1);
...
int git_config_iterator_new(git_config_iterator **out, const git_config *cfg)
{
	...
	iter = (all_iter*) git__calloc(1, sizeof(all_iter));
...
int git_config_delete_entry(git_config *cfg, const char *name)
{
	...
	internal = (file_internal*) git_vector_get(&cfg->files, 0);
	if (!internal || !internal->file)
...
int git_config_set_int64(git_config *cfg, const char *name, int64_t value)
{
  ...
  #ifdef _BCB
	p_snprintf(str_value, sizeof(str_value), "%I64d", value);
  #else %I64d
...
int git_config_set_string(git_config *cfg, const char *name, const char *value)
{
	...
	if (!error && GIT_REFCOUNT_OWNER(cfg) != NULL)
		git_repository__cvar_cache_clear((git_repository*)GIT_REFCOUNT_OWNER(cfg));
...
int git_config_multivar_iterator_new(git_config_iterator **out, const git_config *cfg, const char *name, const char *regexp)
{
	...
	iter = (multivar_iter*) git__calloc(1, sizeof(multivar_iter));
	GITERR_CHECK_ALLOC(iter);
