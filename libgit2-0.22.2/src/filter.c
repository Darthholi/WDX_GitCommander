...
static void filter_registry_shutdown(void)
{
	...
	if ((reg = (filter_registry *) git__swap(git__filter_registry, NULL)) == NULL)
		return;

	git_vector_foreach(&reg->filters, pos, fdef, git_filter_def*) {
...
static int filter_registry_initialize(void)
{
	...
	reg = (filter_registry *) git__calloc(1, sizeof(struct filter_registry));
	...
	reg = (filter_registry *) git__compare_and_swap(&git__filter_registry, NULL, reg);
	if (reg != NULL)
		goto cleanup;
...
static int filter_def_name_key_check(const void *key, const void *fdef)
{
	...
	return name ? git__strcmp((char*)key, name) : -1;
}
...
static git_filter_def *filter_registry_lookup(size_t *pos, const char *name)
{
	...
	if (!filter_registry_find(pos, name))
		fdef = (git_filter_def *) git_vector_get(&git__filter_registry->filters, *pos);
	...
	const char *name, git_filter *filter, int priority)
{
	...
	fdef = (git_filter_def *) git__calloc(
	...
	git_filter_list **out, const git_filter_source *src)
{
	...
	fl = (git_filter_list *) git__calloc(1, sizeof(git_filter_list) + pathlen + 1);
	...
	const char ***out, git_filter_def *fdef, const git_filter_source *src)
{
	int error;
	size_t i;
	const char **strs = (const char **) git__calloc(fdef->nattrs, sizeof(const char *));
	...
	uint32_t options)
{
	...
	git_vector_foreach(&git__filter_registry->filters, idx, fdef, git_filter_def *) {
		...
		if (error == GIT_PASSTHROUGH)
			...
			fe = (git_filter_entry *) git_array_alloc(fl->filters);
