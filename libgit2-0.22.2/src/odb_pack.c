...
static int packfile_sort__cb(const void *a_, const void *b_)
{
	const struct git_pack_file *a = (git_pack_file *) a_;
	const struct git_pack_file *b = (git_pack_file *) b_;
	int st;
...
static int pack_backend__foreach(git_odb_backend *_backend, git_odb_foreach_cb cb, void *data)
{
	...
	git_vector_foreach(&backend->packs, i, p, git_pack_file*) {
	...
	void *progress_payload)
{
	...
	*out = NULL;
	...
	writepack = (pack_writepack*) git__calloc(1, sizeof(struct pack_writepack));
...
{
	struct pack_backend *backend = (pack_backend*) git__calloc(1, sizeof(struct pack_backend));
