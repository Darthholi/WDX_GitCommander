...
static int rebase_open_merge(git_rebase *rebase)
{
	...
	/* Read cmt.* */
	git_array_init_to_size(rebase->operations, end, git_rebase_operation*);
	GITERR_CHECK_ARRAY(rebase->operations);

	for (i = 0; i < end; i++) {
		operation = (git_rebase_operation*) git_array_alloc(rebase->operations);
...
int git_rebase_open(git_rebase **out, git_repository *repo)
{
	...
	rebase = (git_rebase*) git__calloc(1, sizeof(git_rebase));
