...
{
	git_diff_driver_registry *reg = (git_diff_driver_registry *)
...
void git_diff_driver_registry_free(git_diff_driver_registry *reg)
{
	...
	git_strmap_foreach_value(reg->drivers, drv, git_diff_driver*, git_diff_driver_free(drv));
	...
	git_diff_driver *drv, const char *regex_str, int regex_flags)
{
	...
	for (scan = regex_str; scan; scan = end) {
		...
		if ((pat = (git_diff_driver_pattern *) git_array_alloc(drv->fn_patterns)) == NULL) {
			error = -1;
			break;
		}
...
{
	return diff_driver_add_patterns((git_diff_driver*)payload, entry->value, REG_EXTENDED);
}

static int diff_driver_funcname(const git_config_entry *entry, void *payload)
{
	return diff_driver_add_patterns((git_diff_driver*)payload, entry->value, 0);
}
	...
	git_repository *repo)
{
	if (!repo->diff_drivers) {
		...
		reg = (git_diff_driver_registry *) git__compare_and_swap(&repo->diff_drivers, NULL, reg);
	...
	const char *driver_name)
{
	...
	drv = (git_diff_driver *) git__calloc(1, sizeof(git_diff_driver) + namelen + 1);
	...
	git_diff_driver **out, git_repository *repo, const char *driver_name)
{
	...
	if (git_strmap_valid_index(reg->drivers, pos)) {
		*out = (git_diff_driver *) git_strmap_value_at(reg->drivers, pos);
		return 0;
	}
...
static long diff_context_find(
	...
	void *payload)
{
	git_diff_find_context_payload *ctxt = (git_diff_find_context_payload *) payload;
