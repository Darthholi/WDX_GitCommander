	...
	const char *name)
{
	...
	*value = NULL;
	...
	git_vector_foreach(&files, i, file, git_attr_file*) {

		git_attr_file__foreach_matching_rule(file, &path, j, rule, git_attr_rule*) {
			size_t pos;
	...
	const char **names)
{
	...
	info = (attr_get_many_info *) git__calloc(num_attr, sizeof(attr_get_many_info));
	...
	void *payload)
{
	...
	git_vector_foreach(&files, i, file, git_attr_file *) {
		...
		git_attr_file__foreach_matching_rule(file, &path, j, rule, git_attr_rule *) {
			...
			git_vector_foreach(&rule->assigns, k, assign, git_attr_assignment *) {
	...
	const char *values)
{
	...
	macro = (git_attr_rule*) git__calloc(1, sizeof(git_attr_rule));
