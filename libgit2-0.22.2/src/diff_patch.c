	...
	git_patch *patch, git_diff *diff, size_t delta_index)
{
	...
	patch->diff  = diff;
	patch->delta = (git_diff_delta*) git_vector_get(&diff->deltas, delta_index);
...
{
	int error;
	git_patch *patch = (git_patch*) git__calloc(1, sizeof(git_patch));
	...
	void *payload)
{
	...
	git_vector_foreach(&diff->deltas, idx, patch.delta, git_diff_delta*) {
	...
	const git_diff_options *opts)
{
	...
	if (opts && (opts->flags & GIT_DIFF_REVERSE) != 0) {
		void *tmp = lfile; lfile = rfile; rfile = (git_diff_file*) tmp;
		tmp = ldata; ldata = rdata; rdata = (git_diff_file_content*) tmp;
	}
	...
	const char **new_path)
{
	...
	*out = pd = (diff_patch_with_delta*) git__calloc(1, sizeof(*pd) + old_len + new_len + 2);
	...
	git_patch **patch_ptr, git_diff *diff, size_t idx)
{
	...
	delta = (git_diff_delta*) git_vector_get(&diff->deltas, idx);
	if (!delta) {
...
{
	*ptr = (char*) patch->ofile.map.data;
	*len = (size_t) patch->ofile.map.len;
}

void git_patch__new_data(
	char **ptr, size_t *len, git_patch *patch)
{
	*ptr = (char*) patch->nfile.map.data;
	*len = (size_t) patch->nfile.map.len;
}
...
static int diff_patch_hunk_cb(
	...
	void *payload)
{
	git_patch *patch = (git_patch*) payload;
	diff_patch_hunk *hunk;

	GIT_UNUSED(delta);

	hunk = (diff_patch_hunk*) git_array_alloc(patch->hunks);
	...
	void *payload)
{
	...
	line = (git_diff_line*) git_array_alloc(patch->lines);
