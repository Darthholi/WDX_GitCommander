	...
	git_otype type)
{
	...
	*object_out = NULL;
	...
	object = (git_object*) git__calloc(1, object_size);
	...
	if ((error = def->parse(object, odb_obj)) < 0)
		...
		*object_out = (git_object*) git_cache_store_parsed(&repo->objects, object);
	...
	git_otype type)
{
	...
	if (len == GIT_OID_HEXSZ) {
		 ...
		 */
		cached = (git_cached_obj*) git_cache_get_any(&repo->objects, id);
		if (cached != NULL) {
