...
{
	git_commit *commit = (git_commit*) _commit;
...
{
	commit_parent_oids *data = (commit_parent_oids *) payload;
...
{
	commit_parent_data *data = (commit_parent_data *) payload;
	const git_commit *commit;
	if (curr >= data->total)
		return NULL;
...
{
	const git_commit *commit_to_amend = (git_commit *) payload;
...
{
	git_commit *commit = (git_commit *) _commit;
	const char *buffer_start = (char*) git_odb_object_data(odb_obj), *buffer;
	...
	git_array_init_to_size(commit->parent_ids, 1, git_oid*);
	...
	while (git_oid__parse(&parent_id, &buffer, buffer_end, "parent ") == 0) {
		git_oid *new_id = (git_oid*) git_array_alloc(commit->parent_ids);
		GITERR_CHECK_ALLOC(new_id);

		git_oid_cpy(new_id, &parent_id);
	}

	commit->author = (git_signature*) git__malloc(sizeof(git_signature));
	...
	commit->committer = (git_signature*) git__malloc(sizeof(git_signature));
