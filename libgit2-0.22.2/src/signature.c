...
int git_signature_new(git_signature **sig_out, const char *name, const char *email, git_time_t time, int offset)
{
	...
	*sig_out = NULL;
	...
	p = (git_signature *) git__calloc(1, sizeof(git_signature));
...
int git_signature_dup(git_signature **dest, const git_signature *source)
{
	...
	signature = (git_signature *) git__calloc(1, sizeof(git_signature));
...
int git_signature__pdup(git_signature **dest, const git_signature *source, git_pool *pool)
{
	...
	signature = (git_signature *) git_pool_mallocz(pool, sizeof(git_signature));
	GITERR_CHECK_ALLOC(signature);
		...
		const char *buffer_end, const char *header, char ender)
{
	...
	if ((buffer_end = (char*) memchr(buffer, ender, buffer_end - buffer)) == NULL)
	...
	email_start = (char*) git__memrchr(buffer, '<', buffer_end - buffer);
	email_end = (char*) git__memrchr(buffer, '>', buffer_end - buffer);
