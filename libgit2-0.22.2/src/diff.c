...
static git_diff_delta *diff_delta__alloc(
	...
	const char *path)
{
	git_diff_delta *delta = (git_diff_delta *) git__calloc(1, sizeof(git_diff_delta));
	if (!delta)
		return NULL;
...
{
	git_diff_delta *delta = (git_diff_delta *) git_vector_last(&diff->deltas);
	if (!delta)
		return NULL;
...
{
	const git_diff_delta *da = (git_diff_delta *) a, *db = (git_diff_delta *) b;
	...
	git_iterator *new_iter)
{
	...
	git_diff *diff = (git_diff*) git__calloc(1, sizeof(git_diff));
	if (!diff)
		return NULL;
...
{
	assert(diff);
	return (git_diff_delta *)git_vector_get(&diff->deltas, idx);
}
	...
	void *payload)
{
	...
	for (i = 0, j = 0; i < i_max || j < j_max; ) {
		h2i = (git_diff_delta *) (head2idx ? GIT_VECTOR_GET(&head2idx->deltas, i) : NULL);
		i2w = (git_diff_delta *) (idx2wd ? GIT_VECTOR_GET(&idx2wd->deltas, j) : NULL);
	...
	const git_diff_format_email_options *opts)
{
	...
	if ((loc = (char*) strpbrk(opts->summary, "\r\n")) != NULL) {
		size_t offset = 0;
		...
		summary = (char*) git__calloc(offset + 1, sizeof(char));
