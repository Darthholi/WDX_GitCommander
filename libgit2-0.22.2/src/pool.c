...
static void *pool_alloc_page(git_pool *pool, uint32_t size)
{
	...
	page = (git_pool_page*) git__calloc(1, alloc_size + sizeof(git_pool_page));
	if (!page)
		return NULL;
...
{
	struct pool_freelist *item = (pool_freelist *) ptr;

	assert(pool && pool->item_size >= sizeof(void*));

	if (item) {
		item->next = (pool_freelist *) pool->free_list;
		pool->free_list = item;
	}
}
...
void git_pool_free_array(git_pool *pool, size_t count, void **ptrs)
{
	...
	items[i]->next  = (pool_freelist *) pool->free_list;
