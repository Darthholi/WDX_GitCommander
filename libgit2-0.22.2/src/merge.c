...
int git_merge_bases_many(git_oidarray *out, git_repository *repo, size_t length, const git_oid input_array[])
{
	...
	list = result;
	while (list) {
		git_oid *id = (git_oid *) git_array_alloc(array);
		if (id == NULL) {
			error = -1;
			goto cleanup;
		}
...
static int interesting(git_pqueue *list)
{
	...
	for (i = 0; i < git_pqueue_size(list); i++) {
		git_commit_list_node *commit = (git_commit_list_node *) git_pqueue_get(list, i);
...
int git_merge__bases_many(git_commit_list **out, git_revwalk *walk, git_commit_list_node *one, git_vector *twos)
{
	...
	git_vector_foreach(twos, i, two, git_commit_list_node *) {
		if (one == two)
	...
	while (interesting(&list)) {
		git_commit_list_node *commit = (git_commit_list_node *) git_pqueue_pop(&list);
		int flags;
	...
	const git_merge_diff *conflict)
{
	...
	*resolved = 0;
	...
	if ((merged = (git_index_entry*) git_pool_malloc(&diff_list->pool, sizeof(git_index_entry))) == NULL)
		return -1;
	...
	unsigned int merge_file_favor)
{
	...
	*resolved = 0;
	...
	if ((index_entry = (git_index_entry*) git_pool_malloc(&diff_list->pool, sizeof(git_index_entry))) == NULL)
	...
	const git_merge_options *opts)
{
	...
	*out = NULL;
	...
	error = opts->metric->buffer_signature(out, &diff_file,
		(char*)git_blob_rawcontent(blob), (size_t)blobsize,
		opts->metric->payload);
	...
	const git_merge_options *opts)
{
	...
	git_vector_foreach(&diff_list->conflicts, i, conflict_src, git_merge_diff *) {
		...
		git_vector_foreach(&diff_list->conflicts, j, conflict_tgt, git_merge_diff *) {
	...
	const git_merge_options *opts)
{
	...
	if (ours_renamed)
		ours_source = (git_merge_diff *) diff_list->conflicts.contents[ours_source_idx];

	if (theirs_renamed)
		theirs_source = (git_merge_diff *) diff_list->conflicts.contents[theirs_source_idx];
	...
	const git_merge_options *opts)
{
	...
	for (i = 0; i < diff_list->conflicts.length; i++) {
		target = (git_merge_diff *) diff_list->conflicts.contents[i];
...
{
	git_merge_diff *conflict = (git_merge_diff *) conflicts->contents[idx];
	...
	size_t *tgt_count)
{
	...
	*tgt_count = 0;
	...
	git_vector_foreach(&diff_list->conflicts, i, entry, git_merge_diff *) {
	...
	const git_merge_options *opts)
{
	...
	similarity_ours = (merge_diff_similarity *) git__calloc(diff_list->conflicts.length,
		sizeof(struct merge_diff_similarity));
	GITERR_CHECK_ALLOC(similarity_ours);

	similarity_theirs = (merge_diff_similarity *) git__calloc(diff_list->conflicts.length,
		sizeof(struct merge_diff_similarity));
	...
	if (diff_list->conflicts.length <= opts->target_limit) {
		...
		cache = (void**) git__calloc(cache_size, sizeof(void *));
	...
	const git_index_entry **entries)
{
	...
	if ((conflict = (git_merge_diff*) git_pool_malloc(pool, sizeof(git_merge_diff))) == NULL)
		return NULL;
	...
	const git_index_entry *tree_items[3])
{
	...
	entry = (git_index_entry*) git_pool_malloc(&diff_list->pool, sizeof(git_index_entry));
...
{
	git_merge_diff_list *diff_list = (git_merge_diff_list *) git__calloc(1, sizeof(git_merge_diff_list));
	...
	const git_merge_options *given)
{
	...
	if (!opts->metric) {
		opts->metric = (git_diff_similarity_metric*) git__malloc(sizeof(git_diff_similarity_metric));
...
int index_from_diff_list(git_index **out, git_merge_diff_list *diff_list)
{
	...
	*out = NULL;
	...
	git_vector_foreach(&diff_list->staged, i, entry, git_index_entry*) {
		if ((error = git_index_add(index, entry)) < 0)
			goto on_error;
	}

	git_vector_foreach(&diff_list->conflicts, i, conflict, git_merge_diff*) {
		const git_index_entry *ancestor =
	...
	git_vector_foreach(&diff_list->resolved, i, conflict, git_merge_diff*) {
		const git_index_entry *ancestor =
			GIT_MERGE_INDEX_ENTRY_EXISTS(conflict->ancestor_entry) ?
			&conflict->ancestor_entry : NULL;
	...
	const git_merge_options *given_opts)
{
	...
	*out = NULL;
	...
	git_vector_foreach(&changes, i, conflict, git_merge_diff*) {
		int resolved = 0;
	...
	git_vector *entries)
{
		...
		git__strncmp(GIT_REFS_HEADS_DIR, entry->merge_head->ref_name, strlen(GIT_REFS_HEADS_DIR)) == 0)
	{
		...
		existing = (merge_msg_entry*) git_vector_get(entries, 0);
	...
	char sep)
{
	...
	git_vector_foreach(entries, i, entry, merge_msg_entry*) {
		if (i > 0 &&
	...
	size_t heads_len)
{
	...
	entries = (merge_msg_entry *) git__calloc(heads_len, sizeof(struct merge_msg_entry));
	...
	size_t their_heads_len)
{
	...
	oids = (git_oid*) git__calloc(their_heads_len + 1, sizeof(git_oid));
...
static int merge_check_index(size_t *conflicts, git_repository *repo, git_index *index_new, git_vector *merged_paths)
{
	...
	*conflicts = 0;
	...
	git_vector_foreach(&staged_diff_list->deltas, i, delta, git_diff_delta *) {
...
int git_merge__check_result(git_repository *repo, git_index *index_new)
{
	...
	git_vector_foreach(&merged_list->deltas, i, delta, git_diff_delta *) {
	...
	for (i = 0; i < git_index_entrycount(index_new); i++) {
		...
		if (git_index_entry_stage(e) != 0 &&
			...
			strcmp((char*)git_vector_last(&paths), e->path) != 0)) {
...
static int merge_preference(git_merge_preference_t *out, git_repository *repo)
{
	...
	*out = GIT_MERGE_PREFERENCE_NONE;
	...
	if (git_config_parse_bool(&bool_value, value) == 0) {
		if (!bool_value)
			*out = *out | GIT_MERGE_PREFERENCE_NO_FASTFORWARD;
	} else {
		if (strcasecmp(value, "only") == 0)
			*out = *out | GIT_MERGE_PREFERENCE_FASTFORWARD_ONLY;
	}
	...
	size_t their_heads_len)
{
	...
	*analysis_out = GIT_MERGE_ANALYSIS_NONE;
	...
	if (git_repository_head_unborn(repo)) {
		*analysis_out = *analysis_out | GIT_MERGE_ANALYSIS_FASTFORWARD | GIT_MERGE_ANALYSIS_UNBORN;
		goto done;
	}
	...
	if (ancestor_head && git_oid_equal(
		...
		*analysis_out = *analysis_out | GIT_MERGE_ANALYSIS_UP_TO_DATE;

	/* We're fastforwardable if we're our own common ancestor. */
	else if (ancestor_head && git_oid_equal(
		git_annotated_commit_id(ancestor_head), git_annotated_commit_id(our_head)))
		*analysis_out = *analysis_out | GIT_MERGE_ANALYSIS_FASTFORWARD | GIT_MERGE_ANALYSIS_NORMAL;

	/* Otherwise, just a normal merge is possible. */
	else
		*analysis_out = *analysis_out | GIT_MERGE_ANALYSIS_NORMAL;
	...
	const git_checkout_options *given_checkout_opts)
{
	...
	their_trees = (git_tree**) git__calloc(their_heads_len, sizeof(git_tree *));
