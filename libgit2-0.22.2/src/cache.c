...
void git_cache_dump_stats(git_cache *cache)
{
	...
	kh_foreach_value(cache->map, object, git_cached_obj *, {
		char oid_str[9];
		printf(" %s%c %s (%d)\n",
...
static void clear_cache(git_cache *cache)
{
	...
	kh_foreach_value(cache->map, evict, git_cached_obj *, {
...
static void cache_evict_entries(git_cache *cache)
{
	...
	while (evict_count > 0) {
		...
		if (kh_exist(cache->map, pos)) {
			git_cached_obj *evict = (git_cached_obj *) kh_val(cache->map, pos);
...
static void *cache_get(git_cache *cache, const git_oid *oid, unsigned int flags)
{
	...
	if (pos != kh_end(cache->map)) {
		entry = (git_cached_obj *) kh_val(cache->map, pos);
...
static void *cache_store(git_cache *cache, git_cached_obj *entry)
{
	...
	/* found */
		...
		git_cached_obj *stored_entry = (git_cached_obj *) kh_val(cache->map, pos);
...
{
	return (git_odb_object *)cache_get(cache, oid, GIT_CACHE_STORE_RAW);
}

git_object *git_cache_get_parsed(git_cache *cache, const git_oid *oid)
{
	return (git_object *)cache_get(cache, oid, GIT_CACHE_STORE_PARSED);
}
...
{
	git_cached_obj *obj = (git_cached_obj*) _obj;
