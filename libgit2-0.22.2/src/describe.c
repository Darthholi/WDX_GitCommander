	...
	const git_oid *sha1)
{
	...
	if (replace_name(&tag, repo, e, prio, sha1)) {
		if (!found) {
			e = (commit_name *) git__malloc(sizeof(struct commit_name));
...
static int commit_name_dup(struct commit_name **out, struct commit_name *in)
{
	...
	name = (commit_name *) git__malloc(sizeof(struct commit_name));
...
static int possible_tag_dup(struct possible_tag **out, struct possible_tag *in)
{
	...
	tag = (possible_tag*) git__malloc(sizeof(struct possible_tag));
	...
	while (git_pqueue_size(list) > 0) {
		...
		if (c->flags & best->flag_within) {
			...
			while (git_pqueue_size(list) > index) {
				git_commit_list_node *i = (git_commit_list_node *) git_pqueue_get(list, index);
	...
	while (git_pqueue_size(&list) > 0)
	{
		...
		if (n) {
			if (!tags && !all && n->prio < 2) {
				...
				struct possible_tag *t = (possible_tag*) git__malloc(sizeof(struct commit_name));
		...
		for (cur_match = 0; cur_match < match_cnt; cur_match++) {
			struct possible_tag *t = (possible_tag*) git_vector_get(&all_matches, cur_match);
...
cleanup:
	{
		...
		git_vector_foreach(&all_matches, i, match, possible_tag *) {
			git__free(match);
		}
	}
	...
	git_describe_options *opts)
{
	...
	data.result = (git_describe_result*) git__calloc(1, sizeof(git_describe_result));
...
cleanup:
	...
	git_oidmap_foreach_value(data.names, name, commit_name*, {
