...
{
	const push_spec *push_spec_a = (push_spec *) a, *push_spec_b = (push_spec *) b;
...
{
	const push_status *push_status_a = (push_status *) a, *push_status_b = (push_status *) b;
...
int git_push_new(git_push **out, git_remote *remote)
{
	...
	*out = NULL;
	...
	p = (git_push*) git__calloc(1, sizeof(*p));
...
static int parse_refspec(git_push *push, push_spec **spec, const char *str)
{
	...
	*spec = NULL;
	...
	s = (push_spec*) git__calloc(1, sizeof(*s));
		...
		const char *reflog_message)
{
	...
	push_spec *xpush_spec = NULL;
	git_reference *remote_ref;
	push_status *status;
	int error = 0;

	git_vector_foreach(&push->status, i, status, push_status *) {
		int fire_callback = 1;
		...
		git_vector_foreach(&push->specs, j, xpush_spec,push_spec *) {
			if (!strcmp(xpush_spec->refspec.dst, status->ref))
				break;
		}
		...
		if (status->msg == NULL) {
			...
			if (git_oid_iszero(&xpush_spec->loid)) {
				...
				error = git_reference_create(NULL, push->remote->repo,
							git_buf_cstr(&remote_ref_name), &xpush_spec->loid, 1, signature,
		...
		if (fire_callback && push->remote->callbacks.update_tips) {
			error = push->remote->callbacks.update_tips(git_buf_cstr(&remote_ref_name),
						&xpush_spec->roid, &xpush_spec->loid, push->remote->callbacks.payload);
...
static int revwalk(git_vector *commits, git_push *push)
{
	...
	git_vector_foreach(&push->remote->refs, i, head, git_remote_head*) {
	...
	while ((error = git_revwalk_next(&oid, rw)) == 0) {
		git_oid *o = (git_oid *) git__malloc(GIT_OID_RAWSZ);
		if (!o) {
			error = -1;
			goto on_error;
		}
...
static int queue_objects(git_push *push)
{
	...
	git_vector_foreach(&commits, i, oid, git_oid *) {
...
static int calculate_work(git_push *push)
{
	...
	git_vector_foreach(&push->specs, i, spec, push_spec *) {
		...
		git_vector_foreach(&push->remote->refs, j, head, git_remote_head *) {
