	...
	const git_filter_source *src)
{
	...
	if (git_filter_source_mode(src) == GIT_FILTER_SMUDGE)
		return crlf_apply_to_workdir((crlf_attrs*)*payload, to, from);
	else
		return crlf_apply_to_odb((crlf_attrs*)*payload, to, from, src);
}
...
{
	struct crlf_filter *f = (crlf_filter *) git__calloc(1, sizeof(struct crlf_filter));
	if (f == NULL)
		return NULL;
