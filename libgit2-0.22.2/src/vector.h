...
#define git_vector_foreach(v, iter, elem, typed)	\
	for ((iter) = 0; (iter) < (v)->length && ((elem) = (typed) (v)->contents[(iter)], 1); (iter)++ )

#define git_vector_rforeach(v, iter, elem, typed)	\
	for ((iter) = (v)->length - 1; (iter) < SIZE_MAX && ((elem) = (typed) (v)->contents[(iter)], 1); (iter)-- )

int git_vector_insert(git_vector *v, void *element);
int git_vector_insert_sorted(git_vector *v, void *element,int (*on_dup)(void **old, void *pnew));
