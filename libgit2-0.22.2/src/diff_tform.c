...
{
	git_diff_delta *delta = (git_diff_delta *) git__malloc(sizeof(git_diff_delta));
	if (!delta)
		return NULL;
...
int git_diff_merge(git_diff *onto, const git_diff *from)
{
	...
	for (i = 0, j = 0; i < onto->deltas.length || j < from->deltas.length; ) {
		git_diff_delta *o = (git_diff_delta *) GIT_VECTOR_GET(&onto->deltas, i);
		const git_diff_delta *f = (git_diff_delta *) GIT_VECTOR_GET(&from->deltas, j);
		int cmp = !f ? -1 : !o ? 1 :
...
{
	GIT_UNUSED(payload);
	*score = git_hashsig_compare((git_hashsig*)siga, (git_hashsig*)sigb);
	return 0;
}
	...
	git_diff *diff, size_t expected_size, bool actually_split)
{
	...
	git_vector_foreach(&diff->deltas, i, delta, git_diff_delta *) {
...
{
	git_diff_delta *delta = (git_diff_delta *) git_vector_get(&diff->deltas, idx / 2);
	...
	if (info->src == GIT_ITERATOR_TYPE_WORKDIR) {
		...
		if (error < 0) {
			...
			error = opts->metric->buffer_signature(
				&cache[info->idx], info->file,
				(char*)git_blob_rawcontent(info->blob), sz, opts->metric->payload);
		}
	}
	...
	void **cache)
{
	...
	git_diff_delta *delta = (git_diff_delta *) GIT_VECTOR_GET(&diff->deltas, delta_idx);
	...
	const git_diff_find_options *given_opts)
{
	...
	sigcache = (void**) git__calloc(num_deltas * 2, sizeof(void *));
	 ...
	 */
	git_vector_foreach(&diff->deltas, t, tgt, git_diff_delta *) {
	...
	src2tgt = (diff_find_match*) git__calloc(num_deltas, sizeof(diff_find_match));
	GITERR_CHECK_ALLOC(src2tgt);
	tgt2src = (diff_find_match*) git__calloc(num_deltas, sizeof(diff_find_match));
	GITERR_CHECK_ALLOC(tgt2src);

	if (FLAG_SET(&opts, GIT_DIFF_FIND_COPIES)) {
		tgt2src_copy = (diff_find_match*) git__calloc(num_deltas, sizeof(diff_find_match));
...
find_best_matches:
	...
	git_vector_foreach(&diff->deltas, t, tgt, git_diff_delta*) {
		...
		git_vector_foreach(&diff->deltas, s, src, git_diff_delta*) {
	...
	git_vector_foreach(&diff->deltas, t, tgt, git_diff_delta*) {
		...
		s = best_match->idx;
		src = (git_diff_delta*) GIT_VECTOR_GET(&diff->deltas, s);
		...
		if (src->status == GIT_DELTA_DELETED) {
			...
			best_match = &tgt2src_copy[t];
			src = (git_diff_delta*) GIT_VECTOR_GET(&diff->deltas, best_match->idx);
