...
{
	int error;
	git_tree *subtree = NULL, *pnew = NULL;
	char subtree_name[3];
	...
	error = manipulate_note_in_tree_r(
		&pnew, repo, subtree, note_oid, annotated_object_sha,
	...
	error = tree_write(out, repo, parent, git_tree_id(pnew),
	...
	git_blob *blob)
{
	...
	note->message = git__strndup((char*)git_blob_rawcontent(blob), git_blob_rawsize(blob));
