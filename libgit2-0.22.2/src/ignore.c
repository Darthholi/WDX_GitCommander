...
static int does_negate_rule(int *out, git_vector *rules, git_attr_fnmatch *match)
{
	...
	git_vector_foreach(rules, i, rule, git_attr_fnmatch *) {
	...
	git_repository *repo, git_attr_file *attrs, const char *data)
{
	...
	while (!error && *scan) {
		...
		if (!match && !(match = (git_attr_fnmatch *) git__calloc(1, sizeof(*match)))) {
			error = -1;
			break;
		}
...
{
	git_ignores *ign = (git_ignores *) payload;
	ign->depth++;
...
int git_ignore__pop_dir(git_ignores *ign)
{
	if (ign->ign_path.length > 0) {
		git_attr_file *file = (git_attr_file *) git_vector_last(&ign->ign_path);
...
void git_ignore__free(git_ignores *ignores)
{
	...
	git_vector_foreach(&ignores->ign_path, i, file, git_attr_file *) {
		git_attr_file__free(file);
		ignores->ign_path.contents[i] = NULL;
	}
	git_vector_free(&ignores->ign_path);

	git_vector_foreach(&ignores->ign_global, i, file, git_attr_file *) {
		git_attr_file__free(file);
	...
	int *ignored, git_attr_file *file, git_attr_path *path)
{
	...
	git_vector_rforeach(&file->rules, j, match, git_attr_fnmatch *) {
