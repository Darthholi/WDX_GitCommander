...
static git_tree_entry *alloc_entry(const char *filename)
{
	...
	entry = (git_tree_entry *) git__malloc(sizeof(git_tree_entry) + filename_len + 1);
	if (!entry)
		return NULL;
...
{
	const struct tree_key_search *ksearch = (tree_key_search *) key;
	const git_tree_entry *entry = (git_tree_entry *) array_member;
	...
	size_t *at_pos, git_vector *entries, const char *filename, size_t filename_len)
{
	...
	for (i = homing; i < entries->length; ++i) {
		entry = (git_tree_entry *) entries->contents[i];
...
int git_tree_entry_dup(git_tree_entry **dest, const git_tree_entry *source)
{
	...
	copy = (git_tree_entry *) git__malloc(total_size);
...
{
	git_tree *tree = (git_tree *) _tree;
	size_t i;
	git_tree_entry *e;

	git_vector_foreach(&tree->entries, i, e, git_tree_entry *)
	...
	const git_tree *tree, const char *name, size_t name_len)
{
	...
	return (git_tree_entry *)git_vector_get(&tree->entries, idx);
}
...
int git_tree__prefix_position(const git_tree *tree, const char *path)
{
	...
	for (; at_pos < entries->length; ++at_pos) {
		const git_tree_entry *entry = (git_tree_entry *) entries->contents[at_pos];
		if (homing_search_cmp(&ksearch, entry) < 0)
			break;
	}

	for (; at_pos > 0; --at_pos) {
		const git_tree_entry *entry = (git_tree_entry *) entries->contents[at_pos - 1];
		if (homing_search_cmp(&ksearch, entry) > 0)
			break;
	}
...
{
	git_tree *tree = (git_tree*) _tree;
	const char *buffer = (char*) git_odb_object_data(odb_obj);
	...
	const git_tree *source)
{
	...
	bld = (git_treebuilder*) git__calloc(1, sizeof(git_treebuilder));
	...
	if (source != NULL) {
		...
		git_vector_foreach(&source->entries, i, entry_src,git_tree_entry *) {
			if (append_entry(
	...
	git_filemode_t filemode)
{
	...
	if (git_strmap_valid_index(bld->map, pos)) {
		entry = (git_tree_entry *) git_strmap_value_at(bld->map, pos);
	} else {
		entry = alloc_entry(filename);
		GITERR_CHECK_ALLOC(entry);
...
int git_treebuilder_write(git_oid *oid, git_treebuilder *bld)
{
	...
	git_strmap_foreach_value(bld->map, entry, git_tree_entry *, {
	...
	for (i = 0; i < entries.length && !error; ++i) {
		git_tree_entry *entry = (git_tree_entry *) git_vector_get(&entries, i);
	...
	void *payload)
{
	...
	git_strmap_foreach(bld->map, filename, entry, git_tree_entry *, {
...
void git_treebuilder_clear(git_treebuilder *bld)
{
	...
	git_strmap_foreach_value(bld->map, e, git_tree_entry *, git_tree_entry_free(e));
	...
	bool preorder)
{
	...
	git_vector_foreach(&tree->entries, i, entry,git_tree_entry *) {
		if (preorder) {
