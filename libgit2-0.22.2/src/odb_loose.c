...
static size_t get_object_header(obj_hdr *hdr, unsigned char *data)
{
	...
	while ((c = data[used]) != ' ') {
		ttypename[used++] = c;
		if (used >= sizeof(ttypename))
			return 0;
	}
	ttypename[used] = 0;
	if (used == 0)
		return 0;
	hdr->type = git_object_string2type(ttypename);
...
{
	unsigned char *buf, *head = (unsigned char*) hb;
	size_t tail;
	 ...
	 * head buffer, if any.
	 */
	if ((buf = (unsigned char*) git__malloc(hdr->size + 1)) == NULL) {
		inflateEnd(s);
		return NULL;
	}
...
static int inflate_packlike_loose_disk_obj(git_rawobj *out, git_buf *obj)
{
	 ...
	 */
	buf = (unsigned char*) git__malloc(hdr.size + 1);
...
static int inflate_disk_obj(git_rawobj *out, git_buf *obj)
{
	 ...
	 */
	if ((buf = (unsigned char*) inflate_tail(&zs, head, used, &hdr)) == NULL)
		return -1;
	buf[hdr.size] = '\0';
...
static int loose_backend__stream(git_odb_stream **stream_out, git_odb_backend *_backend, size_t length, git_otype type)
{
	...
	*stream_out = NULL;
	...
	stream = (loose_writestream*) git__calloc(1, sizeof(loose_writestream));
	...
	unsigned int file_mode)
{
	...
	backend = (loose_backend*) git__calloc(1, sizeof(loose_backend) + objects_dirlen + 2);
