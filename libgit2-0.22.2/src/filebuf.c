...
int git_filebuf_open(git_filebuf *file, const char *path, int flags, mode_t mode)
{
	...
	if (!file->do_not_buffer) {
		file->buffer = (unsigned char*) git__malloc(file->buf_size);
	...
	if (compression != 0) {
		...
		file->z_buf = (unsigned char*) git__malloc(file->buf_size);
	...
	if (flags & GIT_FILEBUF_TEMPORARY) {
		...
		file->path_lock = (char*) git__malloc(path_len + GIT_FILELOCK_EXTLENGTH);
		GITERR_CHECK_ALLOC(file->path_lock);
...
{
	const unsigned char *buf = (unsigned char*) buff;
...
int git_filebuf_printf(git_filebuf *file, const char *format, ...)
{
	...
	tmp_buffer = (char*) git__malloc(len + 1);
	if (!tmp_buffer) {
		file->last_error = BUFERR_MEM;
		return -1;
	}
