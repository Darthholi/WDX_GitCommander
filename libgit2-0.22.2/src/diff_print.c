...
{
	diff_print_info *pi = (diff_print_info *) data;
	git_buf *out = pi->buf;
	...
	const char *ptemplate)
{
	...
	return git_buf_printf(out, ptemplate, oldpfx, oldpath, newpfx, newpath);
}
...
static int print_binary_hunk(diff_print_info *pi, git_blob *old, git_blob *pnew)
{
	...
	new_data = pnew ? git_blob_rawcontent(pnew) : NULL;

	old_data_len = old ? git_blob_rawsize(old) : 0;
	new_data_len = pnew ? git_blob_rawsize(pnew) : 0;
	...
	const char *oldpfx, const char *newpfx)
{
	...
	if (old_id && (error = git_blob_lookup(&old, pi->diff->repo, old_id)) < 0)
		goto done;
	if (new_id && (error = git_blob_lookup(&pnew, pi->diff->repo,new_id)) < 0)
		goto done;

	if ((error = print_binary_hunk(pi, old, pnew)) < 0 ||
		(error = git_buf_putc(pi->buf, '\n')) < 0 ||
		(error = print_binary_hunk(pi, pnew, old)) < 0)
	{
		if (error == GIT_EBUFS) {
			giterr_clear();
...
int git_diff_print_callback__to_buf(
	...
	void *payload)
{
	git_buf *output = (git_buf*)payload;
...
int git_diff_print_callback__to_file_handle(
	...
	void *payload)
{
	FILE *fp = (FILE*) (payload ? payload : stdout);
