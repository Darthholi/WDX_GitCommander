...
#define git_array_init_to_size(a, desired, typed) \
	do { (a).size = 0; (a).asize = desired; (a).ptr = (typed) git__calloc(desired, sizeof(*(a).ptr)); } while (0)
...
GIT_INLINE(void *) git_array_grow(void *_a, size_t item_size)
{
	...
	char *new_array = (char*) git__realloc(a->ptr, new_size * item_size);
	if (!new_array) {
		git_array_clear(*a);
		return NULL;
	} else {
