...
static unsigned int index_delta2status(const git_diff_delta *head2idx)
{
	...
	case GIT_DELTA_RENAMED:
		...
		if (!git_oid_equal(&head2idx->old_file.id, &head2idx->new_file.id))
			st = st | GIT_STATUS_INDEX_MODIFIED;
		break;
	...
	git_diff_delta *idx2wd)
{
	...
	if (head2idx)
		st = st | index_delta2status(head2idx);

	if (idx2wd)
		st = st | workdir_delta2status(status->idx2wd, idx2wd);
...
static int status_collect(
	...
	void *payload)
{
	git_status_list *status = (git_status_list *) payload;
	git_status_entry *status_entry;

	if (!status_is_included(status, head2idx, idx2wd))
		return 0;

	status_entry = (git_status_entry *) git__malloc(sizeof(git_status_entry));
...
{
	const git_status_entry *entry_a = (git_status_entry *) a;
	const git_status_entry *entry_b = (git_status_entry *) b;
...
static git_status_list *git_status_list_alloc(git_index *index)
{
	...
	if (!(status = (git_status_list *) git__calloc(1, sizeof(git_status_list))))
		return NULL;
...
const git_status_entry *git_status_byindex(git_status_list *status, size_t i)
{
	...
	return (git_status_entry *)git_vector_get(&status->paired, i);
}
	...
	void *payload)
{
	...
	git_vector_foreach(&status->paired, i, status_entry,git_status_entry *) {
...
{
	struct status_file_info *sfi = (status_file_info *) data;
	int (*strcomp)(const char *a, const char *b);
