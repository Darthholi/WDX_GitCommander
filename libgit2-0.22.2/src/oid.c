...
{
	char *str = (char*) git__malloc(GIT_OID_HEXSZ + 1);
	if (!str)
		return NULL;
	git_oid_nfmt(str, GIT_OID_HEXSZ + 1, oid);
	return str;
...
{
	self->nodes = (trie_node*) git__realloc(self->nodes, new_size * sizeof(trie_node));
...
git_oid_shorten *git_oid_shorten_new(size_t min_length)
{
	...
	os = (git_oid_shorten*) git__calloc(1, sizeof(git_oid_shorten));
	if (os == NULL)
		return NULL;
