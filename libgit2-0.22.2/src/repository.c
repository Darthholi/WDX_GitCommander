...
static void set_odb(git_repository *repo, git_odb *odb)
{
	...
	if ((odb = (git_odb*) git__swap(repo->_odb, odb)) != NULL) {
...
static void set_index(git_repository *repo, git_index *index)
{
	...
	if ((index = (git_index*) git__swap(repo->_index, index)) != NULL) {
...
int git_repository_index__weakptr(git_index **out, git_repository *repo)
{
	...
	if (repo->_index == NULL) {
		...
		if (!error) {
			...
			index = (git_index*) git__compare_and_swap(&repo->_index, NULL, index);
			if (index != NULL) {
				GIT_REFCOUNT_OWN(index, NULL);
				git_index_free(index);
			}
...
int git_repository_set_namespace(git_repository *repo, const char *nnamespace)
{
	git__free(repo->nnamespace);

	if (nnamespace == NULL) {
		repo->nnamespace = NULL;
		return 0;
	}

	return (repo->nnamespace = git__strdup(nnamespace)) ? 0 : -1;
}

const char *git_repository_get_namespace(git_repository *repo)
{
	return repo->nnamespace;
}
