...
int git_transaction_new(git_transaction **out, git_repository *repo)
{
	...
	tx = (git_transaction *) git_pool_mallocz(&pool, sizeof(git_transaction));
	if (!tx) {
		error = -1;
		goto on_error;
	}
...
int git_transaction_lock_ref(git_transaction *tx, const char *refname)
{
	...
	node = (transaction_node *) git_pool_mallocz(&tx->pool, sizeof(transaction_node));
...
static int find_locked(transaction_node **out, git_transaction *tx, const char *refname)
{
	...
	node = (transaction_node *) git_strmap_value_at(tx->locks, pos);
...
static int dup_reflog(git_reflog **out, const git_reflog *in, git_pool *pool)
{
	...
	reflog = (git_reflog*) git_pool_mallocz(pool, sizeof(git_reflog));
	...
	reflog->entries.contents = (void**) git_pool_mallocz(pool, len * sizeof(void *));
	GITERR_CHECK_ALLOC(reflog->entries.contents);

	entries = (git_reflog_entry*) git_pool_mallocz(pool, len * sizeof(git_reflog_entry));
	...
	for (i = 0; i < len; i++) {
		...
		src = (git_reflog_entry*) git_vector_get(&in->entries, i);
