...
static int tag_parse(git_tag *tag, const char *buffer, const char *buffer_end)
{
	...
	search = (char*) memchr(buffer, '\n', buffer_end - buffer);
	if (search == NULL)
		return tag_error("Object too short");

	text_len = search - buffer;

	tag->tag_name = (char*) git__malloc(text_len + 1);
	...
	if (buffer < buffer_end && *buffer != '\n') {
		tag->tagger = (git_signature*) git__malloc(sizeof(git_signature));
	...
	if (buffer < buffer_end) {
		...
		tag->message = (char*) git__malloc(text_len + 1);
