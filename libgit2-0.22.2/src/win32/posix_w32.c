...
int p_link(const char *old, const char *pnew)
{
	GIT_UNUSED(old);
	GIT_UNUSED(pnew);
	errno = ENOSYS;
...
int p_symlink(const char *old, const char *pnew)
{
	 ...
	 */
	return git_futils_fake_symlink(old, pnew);
}
...
char *p_realpath(const char *orig_path, char *buffer)
{
	...
	if (!buffer && !(buffer = (char*) git__malloc(GIT_WIN_PATH_UTF8))) {
		errno = ENOMEM;
		return NULL;
	}
