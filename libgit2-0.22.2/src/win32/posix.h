...
#ifdef _BCB
#define p_fstat(f,b) _fstati64(f,b)
#else
...
extern int p_symlink(const char *old, const char *pnew);
extern int p_link(const char *old, const char *pnew);
...
#ifdef _BCB
#include <windows\crtl\errno.h>
#include <windows\crtl\dos.h>
#define ENOSYS 38
#include <windows\crtl\dir.h>
#define _chsize(fd, size) chsize(fd, size)
#define strcasecmp(s1, s2) stricmp(s1, s2)
#define strncasecmp(s1, s2, c) strnicmp(s1, s2, c)
#else
