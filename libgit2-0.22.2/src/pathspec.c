	...
	git_vector *vspec, const git_strarray *strspec, git_pool *strpool)
{
	...
	for (i = 0; i < strspec->count; ++i) {
		...
		git_attr_fnmatch *match = (git_attr_fnmatch *) git__calloc(1, sizeof(git_attr_fnmatch));
		if (!match)
			return -1;
	...
	const char *path1)
{
	...
	git_vector_foreach(vspec, i, match, git_attr_fnmatch *) {
	...
	size_t *matched_at)
{
	...
	if (result >= 0) {
		if (matched_pathspec) {
			const git_attr_fnmatch *match = (git_attr_fnmatch *) git_vector_get(vspec, pos);
...
{
	int error = 0;
	git_pathspec *ps = (git_pathspec*) git__malloc(sizeof(git_pathspec));
...
{
	git_pathspec_match_list *m = (git_pathspec_match_list *) git__calloc(1, sizeof(git_pathspec_match_list));
	...
	const char *path1)
{
	...
	for (; start < patterns->length; ++start) {
		const git_attr_fnmatch *pat = (git_attr_fnmatch *) git_vector_get(patterns, start);
	...
	git_pool *pool)
{
	...
	for (pos = 0; pos < patterns->length; ++pos) {
		...
		if ((failed = (char**) git_array_alloc(*failures)) == NULL)
			return -1;

		pat = (git_attr_fnmatch *) git_vector_get(patterns, pos);
...
static git_iterator_flag_t pathspec_match_iter_flags(uint32_t flags)
{
	...
	if ((flags & GIT_PATHSPEC_IGNORE_CASE) != 0)
		f = f | GIT_ITERATOR_IGNORE_CASE;
	else if ((flags & GIT_PATHSPEC_USE_CASE) != 0)
		f = f | GIT_ITERATOR_DONT_IGNORE_CASE;
