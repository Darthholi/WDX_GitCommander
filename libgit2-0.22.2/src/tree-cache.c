			      ...
			      git_pool *pool)
{
	...
	if ((buffer = (const char*) memchr(buffer, '\0', buffer_end - buffer)) == NULL)
		goto corrupted;
	...
	if (tree->children_count > 0) {
		...
		tree->children = (git_tree_cache**) git_pool_malloc(pool, tree->children_count * sizeof(git_tree_cache *));
...
static int read_tree_recursive(git_tree_cache *cache, const git_tree *tree, git_pool *pool)
{
	...
	cache->children = (git_tree_cache**) git_pool_mallocz(pool, ntrees * sizeof(git_tree_cache *));
...
int git_tree_cache_new(git_tree_cache **out, const char *name, git_pool *pool)
{
	...
	tree = (git_tree_cache*) git_pool_malloc(pool, sizeof(git_tree_cache) + name_len + 1);
