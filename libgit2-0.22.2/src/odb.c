...
static git_cache *odb_cache(git_odb *odb)
{
	if (odb->rc.owner != NULL) {
		git_repository *owner = (git_repository *) odb->rc.owner;
...
static int init_fake_wstream(git_odb_stream **stream_p, git_odb_backend *backend, size_t size, git_otype type)
{
	...
	stream = (fake_wstream*) git__calloc(1, sizeof(fake_wstream));
	GITERR_CHECK_ALLOC(stream);

	stream->size = size;
	stream->type = type;
	stream->buffer = (char*) git__malloc(size);
...
int git_odb_get_backend(git_odb_backend **out, git_odb *odb, size_t pos)
{
	...
	assert(out && odb);
	internal = (backend_internal *) git_vector_get(&odb->backends, pos);
...
int git_odb_foreach(git_odb *db, git_odb_foreach_cb cb, void *payload)
{
	...
	git_vector_foreach(&db->backends, i, internal, backend_internal *) {
		git_odb_backend *b = internal->backend;
...
void git_odb_stream_free(git_odb_stream *stream)
{
	...
	git_hash_ctx_cleanup((git_hash_ctx*)stream->hash_ctx);
