...
{
	int error = 0;
	futils__rmdir_data *data = (futils__rmdir_data *) opaque;
	struct stat st;
...
int git_futils_fake_symlink(const char *old, const char *pnew)
{
	int retcode = GIT_ERROR;
	int fd = git_futils_creat_withpath(pnew, 0755, 0644);
	if (fd >= 0) {
...
static int cp_link(const char *from, const char *to, size_t link_size)
{
	int error = 0;
	ssize_t read_len;
	char *link_data = (char*) git__malloc(link_size + 1);
