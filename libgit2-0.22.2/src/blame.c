...
static git_blame_hunk* new_hunk(
		...
		const char *path)
{
	git_blame_hunk *hunk = (git_blame_hunk *) git__calloc(1, sizeof(git_blame_hunk));
	if (!hunk) return NULL;
...
void git_blame_free(git_blame *blame)
{
	...
	git_vector_foreach(&blame->hunks, i, hunk, git_blame_hunk *)
		free_hunk(hunk);
...
static int index_blob_lines(git_blame *blame)
{
    ...
    while (len--) {
        if (bol) {
            i = (size_t*) git_array_alloc(blame->line_index);
...
static int blame_internal(git_blame *blame)
{
	...
	if ((error = load_blob(blame)) < 0 ||
		...
		goto cleanup;
	blame->final_buf = (char*) git_blob_rawcontent(blame->final_blob);
	blame->final_buf_size = git_blob_rawsize(blame->final_blob);

	ent = (git_blame__entry*) git__calloc(1, sizeof(git_blame__entry));
		...
		size_t buffer_len)
{
	...
	git_vector_foreach(&reference->hunks, i, hunk, git_blame_hunk *) {
