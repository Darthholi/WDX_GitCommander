	...
	const char *alternate)
{
	...
	if (out != NULL) {
		git_submodule *sm = (git_submodule *) git_strmap_value_at(cache->submodules, pos);
		GIT_REFCOUNT_INC(sm);
		*out = sm;
	}
	...
	git_submodule_cache *cache, uint32_t mask)
{
	...
	git_strmap_foreach_value(cache->submodules, sm, git_submodule *, {
...
{
	git_submodule_free((git_submodule *)sm);
}
	...
	void *payload)
{
	...
	git_vector_foreach(&snapshot, i, sm, git_submodule *) {
...
void git_submodule_cache_free(git_repository *repo)
{
	...
	if ((cache = (git_submodule_cache*) git__swap(repo->_submodules, NULL)) != NULL)
	...
	git_submodule **out, git_submodule_cache *cache, const char *name)
{
	...
	sm = (git_submodule *) git__calloc(1, sizeof(git_submodule));
	...
	bool free_after_remove)
{
	...
	for (; name; name = alt, alt = NULL) {
		...
		found = (git_submodule *) git_strmap_value_at(map, pos);
	...
	const char *alternate)
{
	...
	if (!git_strmap_valid_index(cache->submodules, pos)) {
		...
		if (error < 0)
			...
			git_submodule_free(sm);
			sm = (git_submodule *) git_strmap_value_at(cache->submodules, pos);
		} else {
			error = 0;
...
{
	git_submodule_cache *cache = (git_submodule_cache *) payload;
	...
	if (alternate) {
		...
		if (old_sm && ((git_submodule *)old_sm) != sm) {
			git_submodule_free((git_submodule *)old_sm);
...
{
	git_submodule_cache *cache = (git_submodule_cache *) git__calloc(1, sizeof(git_submodule_cache));
...
static int submodule_cache_init(git_repository *repo, int cache_refresh)
{
	...
	if (!(error = submodule_cache_alloc(&cache, repo)) &&
		...
		cache = (git_submodule_cache *) git__compare_and_swap(&repo->_submodules, NULL, cache);
