...
{
	const checkout_conflictdata *ca = (checkout_conflictdata *) a;
	const checkout_conflictdata *cb = (checkout_conflictdata *) b;
	int diff;
	...
	const git_vector *conflicts, size_t idx, void *payload)
{
	...
	if ((conflict = (checkout_conflictdata *) git_vector_get(conflicts, idx)) == NULL)
		return -1;
...
static int checkout_conflict_append_update(
	...
	void *payload)
{
	checkout_data *data = (checkout_data *) payload;
	checkout_conflictdata *conflict;
	int error;

	conflict = (checkout_conflictdata *) git__calloc(1, sizeof(checkout_conflictdata));
...
{
	const char *path = (char*) p;
	const checkout_conflictdata *conflict = (checkout_conflictdata *) c;
	...
	const char *path)
{
	...
	return (checkout_conflictdata *)git_vector_get(&data->update_conflicts, pos);
}
	...
	const char *path)
{
	...
	git_vector_foreach(&data->update_conflicts, i, conflict, checkout_conflictdata *) {
		int cmp = -1;
	...
	git_iterator *workdir)
{
	...
	*counts_ptr = counts = (size_t*) git__calloc(CHECKOUT_ACTION__MAX+1, sizeof(size_t));
	*actions_ptr = actions = (uint32_t*) git__calloc(
	...
	git_vector_foreach(deltas, i, delta, git_diff_delta*) {
	...
	checkout_data *data)
{
	...
	git_vector_foreach(&data->diff->deltas, i, delta, git_diff_delta*) {
	...
	git_vector_foreach(&data->removes, i, str, char*) {
	...
	checkout_conflictdata *conflict)
{
	...
	if (data->opts.checkout_strategy & GIT_CHECKOUT_CONFLICT_STYLE_DIFF3)
		opts.flags = opts.flags | GIT_MERGE_FILE_STYLE_DIFF3;
...
static int checkout_remove_conflicts(checkout_data *data)
{
	...
	git_vector_foreach(&data->remove_conflicts, i, conflict, char*) {
...
static int checkout_extensions_update_index(checkout_data *data)
{
	...
	if (data->update_reuc) {
		git_vector_foreach(data->update_reuc, i, reuc_entry, git_index_reuc_entry *) {
	...
	if (data->update_names) {
		git_vector_foreach(data->update_names, i, name_entry, git_index_name_entry *) {
