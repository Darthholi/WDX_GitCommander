...
{
	git_pack_cache_entry *e = (git_pack_cache_entry *) git__calloc(1, sizeof(git_pack_cache_entry));
	if (!e)
		return NULL;
...
static git_pack_cache_entry *cache_get(git_pack_cache *cache, git_off_t offset)
{
	...
	if (k != kh_end(cache->entries)) { /* found it */
		entry = (git_pack_cache_entry *) kh_value(cache->entries, k);
...
static int pack_index_check(const char *path, struct git_pack_file *p)
{
  ...
  if (p_fstat(fd, &st) < 0) {
	...
	//hdr = (git_pack_idx_header*) idx_map = p->index_map.data;
  idx_map = p->index_map.data;
  hdr = (git_pack_idx_header*) idx_map;
...
static int pack_index_open(struct git_pack_file *p)
{
	...
	if ((idx_name = (char*) git__malloc(name_len)) == NULL)
		return -1;
		...
		git_off_t offset)
{
	...
	if (type == GIT_OBJ_OFS_DELTA || type == GIT_OBJ_REF_DELTA) {
		...
		if (error < 0)
			return error;
		error = git__delta_read_header((unsigned char*)delta.data, delta.len, &base_size, size_p);
				 ...
				 struct git_pack_file *p, git_off_t obj_offset)
{
	...
	while (true) {
		...
		if (elem_pos == SMALL_STACK_SIZE) {
			git_array_init_to_size(chain, elem_pos, pack_chain_elem*);
		...
		if (!use_heap) {
			...
			elem = (pack_chain_elem*) git_array_alloc(chain);
			if (!elem) {
				error = -1;
				goto on_error;
			}
	...
	git_off_t *obj_offset)
{
	...
	while (elem_pos > 0 && !error) {
		...
		error = git__delta_apply(obj, (unsigned char*) base.data, base.len, (unsigned char*) delta.data, delta.len);
		obj->type = base_type;
		/*
...
ssize_t git_packfile_stream_read(git_packfile_stream *obj, void *buffer, size_t len)
{
	...
	obj->zstream.next_out = (unsigned char*) buffer;
	...
	git_otype type)
{
	...
	buffer = (unsigned char*) git__calloc(1, size + 1);
...
int git_packfile_alloc(struct git_pack_file **pack_out, const char *path)
{
	...
	*pack_out = NULL;
	...
	p = (git_pack_file*) git__calloc(1, sizeof(*p) + path_len + 2);
...
static git_off_t nth_packed_object_offset(const struct git_pack_file *p, uint32_t n)
{
	const unsigned char *index = (unsigned char*) p->index_map.data;
	index += 4 * 256;
	if (p->index_version == 1) {
...
int git_pack_foreach_entry(
	...
	void *data)
{
	const unsigned char *index = (unsigned char*) p->index_map.data, *current;
	uint32_t i;
	int error = 0;
	...
	if (index == NULL) {
		...
		index = (unsigned char*) p->index_map.data;
	}
	...
	if (p->oids == NULL) {
		...
		if (p->index_version > 1) {
			...
			git_vector_foreach(&offsets, i, current, unsigned char*)
...
static int pack_entry_find_offset(
	...
	size_t len)
{
	const uint32_t *level1_ofs = (uint32_t *) p->index_map.data;
	const unsigned char *index = (unsigned char *) p->index_map.data;
	...
	*offset_out = 0;
	...
	if (p->index_version == -1) {
		...
		index = (unsigned char *) p->index_map.data;
		level1_ofs = (uint32_t*) p->index_map.data;
	}
