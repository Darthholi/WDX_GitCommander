...
{
	const git_commit_list_node *commit_a = (git_commit_list_node *)a;
	const git_commit_list_node *commit_b = (git_commit_list_node *)b;
...
{
	git_commit_list *new_list = (git_commit_list *) git__malloc(sizeof(git_commit_list));
	if (new_list != NULL) {
		new_list->item = item;
	...
	size_t buffer_len)
{
	...
	if ((committer_start = buffer = (char*) memchr(buffer, '\n', buffer_end - buffer)) == NULL)
		return commit_error(commit, "object is corrupted");

	buffer++;

	if ((buffer = (char*) memchr(buffer, '\n', buffer_end - buffer)) == NULL)
		return commit_error(commit, "object is corrupted");
