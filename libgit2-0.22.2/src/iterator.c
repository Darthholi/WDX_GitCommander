...
int git_iterator_for_nothing(
	...
	const char *end)
{
	empty_iterator *i = (empty_iterator *) git__calloc(1, sizeof(empty_iterator));
...
{
	const tree_iterator_entry *ae = (tree_iterator_entry *) a, *be = (tree_iterator_entry *) b;
...
{
	const tree_iterator_frame *tf = (tree_iterator_frame *) key;
...
static int tree_iterator__push_frame(tree_iterator *ti)
{
	...
	tf = (tree_iterator_frame *) git__calloc(sizeof(tree_iterator_frame) +
	...
	for (i = head->current, n_entries = 0; i < head->next; ++i) {
		...
		for (j = 0; j < max_j; ++j) {
			tree_iterator_entry *entry = (tree_iterator_entry *) git_pool_malloc(&ti->pool, 1);
...
static int tree_iterator__create_root_frame(tree_iterator *ti, git_tree *tree)
{
	...
	tree_iterator_frame *root = (tree_iterator_frame *) git__calloc(sz, sizeof(char));
	...
	root->entries[0] = (tree_iterator_entry *) git_pool_mallocz(&ti->pool, 1);
	...
	const char *end)
{
	...
	ti = (tree_iterator *) git__calloc(1, sizeof(tree_iterator));
...
{
	const git_index_entry *ie = (git_index_entry *) git_vector_get(&ii->entries, ii->current);
	...
	if (index_iterator__at_tree(ii)) {
		if (iterator__do_autoexpand(ii)) {
			...
			while (ii->current < entrycount) {
				...
				if (!(ie = (git_index_entry *) git_vector_get(&ii->entries, ii->current)) ||
...
{
	int error = 0;
	index_iterator *ii = (index_iterator *) git__calloc(1, sizeof(index_iterator));
...
{
	fs_iterator_frame *ff = (fs_iterator_frame *) git__calloc(1, sizeof(fs_iterator_frame));
...
static int fs_iterator__entry_cmp(const void *i, const void *item)
{
	...
	const git_path_with_stat *ps = (git_path_with_stat *) item;
	...
	const git_index_entry **entry, git_iterator *self)
{
	...
	while (fi->entry.path != NULL) {
		ff   = fi->stack;
		next = (git_path_with_stat *) git_vector_get(&ff->entries, ++ff->index);
...
static int fs_iterator__update_entry(fs_iterator *fi)
{
	...
	ps = (git_path_with_stat *) git_vector_get(&fi->stack->entries, fi->stack->index);
	if (!ps)
		return GIT_ITEROVER;
...
int git_iterator_for_filesystem(
	...
	const char *end)
{
	fs_iterator *fi = (fs_iterator *) git__calloc(1, sizeof(fs_iterator));
...
static int is_submodule(workdir_iterator *wi, git_path_with_stat *ie)
{
	...
	if (!is_submodule && wi->index) {
		...
		if (!error) {
			e = (git_index_entry*) git_vector_get(&wi->index_snapshot, pos);
...
static int workdir_iterator__enter_dir(fs_iterator *fi)
{
	...
	git_vector_foreach(&ff->entries, pos, entry, git_path_with_stat*) {
	...
	const char *end)
{
	...
	wi = (workdir_iterator*) git__calloc(1, sizeof(workdir_iterator));
