...
git_delta_create_index(const void *buf, unsigned long bufsize)
{
	...
	const unsigned char *data, *buffer = (unsigned char *) buf;
	...
	if (!mem)
		return NULL;
	index = (git_delta_index*) mem;
	mem = index->hash;
	hash = (index_entry**) mem;
	mem = hash + hsize;
	entry = (index_entry*) mem;
	...
	hash_count = (unsigned int*) git__calloc(hsize, sizeof(*hash_count));
	if (!hash_count) {
		git__free(index);
		return NULL;
	}
	...
	unsigned long max_size)
{
	...
	out = (unsigned char*) git__malloc(outsize);
	if (!out)
		return NULL;
	...
	ref_data = (unsigned char*) index->src_buf;
	ref_top = ref_data + index->src_size;
	data = (unsigned char*) trg_buf;
	...
	while (data < top) {
		...
		if (outpos >= outsize - MAX_OP_SIZE) {
			...
			if (max_size && outpos > max_size)
				break;
			out = (unsigned char*) git__realloc(out, outsize);
			if (!out) {
				git__free(tmp);
				return NULL;
			}
