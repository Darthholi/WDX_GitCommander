...
static int append_entry(git_strmap *values, cvar_t *var)
{
	...
	if (!git_strmap_valid_index(values, pos)) {
		...
		existing = (cvar_t*) git_strmap_value_at(values, pos);
...
static void free_vars(git_strmap *values)
{
	...
	git_strmap_foreach_value(values, var, cvar_t*,
		while (var != NULL) {
...
static int refcounted_strmap_alloc(refcounted_strmap **out)
{
	...
	map = (refcounted_strmap*) git__calloc(1, sizeof(refcounted_strmap));
...
static int config_open(git_config_backend *cfg, git_config_level_t level)
{
	...
	reader = (struct reader*) git_array_alloc(b->readers);
	if (!reader) {
	...
	struct git_config_backend* backend)
{
	...
	it = (git_config_file_iter *) git__calloc(1, sizeof(git_config_file_iter));
...
static int config_set(git_config_backend *cfg, const char *name, const char *value)
{
	...
	if (git_strmap_valid_index(values, pos)) {
		cvar_t *existing = (cvar_t *) git_strmap_value_at(values, pos);
...
static int config_get(git_config_backend *cfg, const char *key, const git_config_entry **out)
{
	...
	var = (cvar_t *) git_strmap_value_at(values, pos);
	while (var->next)
		var = var->next;
...
int git_config_file__ondisk(git_config_backend **out, const char *path)
{
	...
	backend = (diskfile_backend *) git__calloc(1, sizeof(diskfile_backend));
...
int git_config_file__snapshot(git_config_backend **out, diskfile_backend *in)
{
	...
	backend = (diskfile_readonly_backend *) git__calloc(1, sizeof(diskfile_readonly_backend));
	GITERR_CHECK_ALLOC(backend);
...
static int reader_peek(struct reader *reader, int flags)
{
	...
	reader->read_ptr = (char*) old_read_ptr;
...
static char *reader_readline(struct reader *reader, bool skip_whitespace)
{
	...
	line = (char*) git__malloc(line_len + 1);
	if (line == NULL)
		return NULL;
...
static int parse_section_header_ext(struct reader *reader, const char *line, const char *base_name, char **section_name)
{
	...
	first_quote = (char*) strchr(line, '"');
	last_quote = (char*) strrchr(line, '"');
...
static int config_parse(git_strmap *values, diskfile_backend *cfg_file, struct reader *reader, git_config_level_t level, int depth)
{
	...
	while (result == 0 && !reader->eof) {
		...
		default: /* assume variable declaration */
			...
			var = (cvar_t*) git__calloc(1, sizeof(cvar_t));
			GITERR_CHECK_ALLOC(var);
			var->entry = (git_config_entry*) git__calloc(1, sizeof(git_config_entry));
			...
			if (!git__strcmp(var->entry->name, "include.path")) {
				...
				r = (struct reader*) git_array_alloc(cfg_file->readers);
...
{
	char *str = (char*) git__malloc(strlen(ptr) + 1);
	char *out = str, *esc;
	...
	while (*ptr != '\0') {
		if (*ptr == '"') {
			...
			*out++ = *ptr;
			...
			if ((esc = (char*) strchr(escapes, *ptr)) != NULL) {
