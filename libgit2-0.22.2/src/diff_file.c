...
static void diff_file_content_binary_by_content(git_diff_file_content *fc)
{
	...
	switch (git_diff_driver_content_is_binary(
		fc->driver, (char*) fc->map.data, fc->map.len)) {
...
int git_diff_file_content__init_from_diff(
	...
	bool use_old)
{
	git_diff_delta *delta = (git_diff_delta *) git_vector_get(&diff->deltas, delta_index);
	bool has_data = true;
	...
	git_diff_file_content *fc, git_buf *path)
{
	...
	read_len = p_readlink((char*)git_buf_cstr(path), (char*)fc->map.data, alloc_len);
	if (read_len < 0) {
