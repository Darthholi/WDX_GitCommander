...
{
	const struct entry_srch_key *srch_key = (entry_srch_key *) key;
	const struct entry_internal *entry = (entry_internal*) array_member;
	int cmp;
...
{
	const git_index_entry *entry = (git_index_entry *) array_member;
...
{
	int diff;
	const git_index_entry *entry_a = (git_index_entry *) a;
	const git_index_entry *entry_b = (git_index_entry *) b;
...
{
	const git_index_name_entry *name_a = (git_index_name_entry *) a;
	const git_index_name_entry *name_b = (git_index_name_entry *) b;
...
{
	const git_index_reuc_entry *reuc = (git_index_reuc_entry *) array_member;

	return strcmp((char*)key, reuc->path);
}
...
static int reuc_isrch(const void *key, const void *array_member)
{
	...
	return strcasecmp((char*)key, reuc->path);
}

static int reuc_cmp(const void *a, const void *b)
{
	const git_index_reuc_entry *info_a = (git_index_reuc_entry *) a;
	const git_index_reuc_entry *info_b = (git_index_reuc_entry *) b;
...
int git_index_open(git_index **index_out, const char *index_path)
{
	...
	index = (git_index*) git__calloc(1, sizeof(git_index));
...
static void index_free_deleted(git_index *index)
{
	...
	for (i = 0; i < index->deleted.length; ++i) {
		git_index_entry *ie = (git_index_entry *) git__swap(index->deleted.contents[i], NULL);
		index_entry_free(ie);
	}
...
{
	int error = 0;
	git_index_entry *entry = (git_index_entry *) git_vector_get(&index->entries, pos);
	...
	git_index *index, size_t n)
{
	...
	if (index_sort_if_needed(index, true) < 0)
		return NULL;
	return (git_index_entry *)git_vector_get(&index->entries, n);
}
	...
	const char *path)
{
	...
	entry = (entry_internal*) git__calloc(sizeof(struct entry_internal) + pathlen + 1, 1);
...
{
	size_t pathlen = strlen(path);
	struct reuc_entry_internal *entry = (reuc_entry_internal *)
	 ...
	 const git_index_entry *entry, size_t pos, int ok_to_replace)
{
	...
	while (pos < index->entries.length) {
		struct entry_internal *p = (entry_internal*) index->entries.contents[pos++];
...
static int index_no_dups(void **old, void *pnew)
{
	const git_index_entry *entry = (git_index_entry *) pnew;
	GIT_UNUSED(old);
...
int git_index_remove_directory(git_index *index, const char *dir, int stage)
{
	...
	while (!error) {
		entry = (git_index_entry *) git_vector_get(&index->entries, pos);
...
int git_index_find(size_t *at_pos, git_index *index, const char *path)
{
	 ...
	 */
	for (; pos > 0; --pos) {
		const git_index_entry *prev = (git_index_entry *) git_vector_get(&index->entries, pos - 1);
	...
	size_t n)
{
	...
	*their_out = NULL;
	...
	for (count = git_index_entrycount(index); n < count; ++n) {
		conflict_entry = (git_index_entry *) git_vector_get(&index->entries, n);
...
static int index_conflict_remove(git_index *index, const char *path)
{
	...
	while ((conflict_entry = (git_index_entry *) git_vector_get(&index->entries, pos)) != NULL) {
...
int git_index_has_conflicts(const git_index *index)
{
	...
	git_vector_foreach(&index->entries, i, entry, git_index_entry*) {
	...
	git_index *index)
{
	...
	it = (git_index_conflict_iterator *) git__calloc(1, sizeof(git_index_conflict_iterator));
	...
	git_index *index, size_t n)
{
	...
	return (git_index_name_entry *)git_vector_get(&index->names, n);
}
	...
	const char *ancestor, const char *ours, const char *theirs)
{
	...
	conflict_name = (git_index_name_entry *) git__calloc(1, sizeof(git_index_name_entry));
...
void git_index_name_clear(git_index *index)
{
	...
	git_vector_foreach(&index->names, i, conflict_name, git_index_name_entry*)
		index_name_entry_free(conflict_name);
	...
	git_index *index, const char *path)
{
	...
	return (git_index_reuc_entry *)git_vector_get(&index->reuc, pos);
}
	...
	git_index *index, size_t n)
{
	...
	git_vector_sort(&index->reuc);
	return (git_index_reuc_entry *)git_vector_get(&index->reuc, n);
}
...
int git_index_reuc_remove(git_index *index, size_t position)
{
	...
	reuc = (git_index_reuc_entry *) git_vector_get(&index->reuc, position);
...
void git_index_reuc_clear(git_index *index)
{
	...
	for (i = 0; i < index->reuc.length; ++i)
		index_entry_reuc_free((git_index_reuc_entry *)git__swap(index->reuc.contents[i], NULL));
...
#define read_conflict_name(ptr) \
	...
	while (size) {
		git_index_name_entry *conflict_name = (git_index_name_entry *) git__calloc(1, sizeof(git_index_name_entry));
		GITERR_CHECK_ALLOC(conflict_name);
	...
	size_t buffer_size)
{
	...
	if (path_length == 0xFFF) {
		...
		path_end = (char*) memchr(path_ptr, '\0', buffer_size);
		if (path_end == NULL)
			return 0;
...
static int read_header(struct index_header *dest, const void *buffer)
{
	const struct index_header *source = (index_header*) buffer;
...
static int write_entries(git_index *index, git_filebuf *file)
{
	...
	git_vector_foreach(entries, i, entry, git_index_entry*)
...
static int write_name_extension(git_index *index, git_filebuf *file)
{
	...
	git_vector_foreach(out, i, conflict_name, git_index_name_entry*) {
...
static int write_reuc_extension(git_index *index, git_filebuf *file)
{
	...
	git_vector_foreach(out, i, reuc, git_index_reuc_entry *) {
...
{
	read_tree_data *data = (read_tree_data *) payload;
	...
	if (data->old_entries != NULL &&
		...
		(old_entry = (git_index_entry*) git_vector_get(data->old_entries, pos)) != NULL &&
	...
	void *payload)
{
	...
	for (i = 0; !error && i < index->entries.length; ++i) {
		git_index_entry *entry = (git_index_entry*) git_vector_get(&index->entries, i);
